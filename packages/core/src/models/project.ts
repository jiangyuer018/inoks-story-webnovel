...
// 每个模型的真实 maxOutput 来自 providers/<name>.ts 的 InoksStoryModel.maxOutput；
...
export const ProseQualityConfigSchema = z.object({
  enabled: z.boolean().default(true),
  enforcement: z.enum(["strict", "balanced", "report-only"]).default("strict"),
  autoRepair: z.boolean().default(true),
  maxRepairIterations: z.number().int().min(0).max(2).default(2),
  minimumScore: z.number().min(0).max(100).default(80),
  failOnUnresolvedBlocking: z.boolean().default(true),
  saveRejectedDraft: z.boolean().default(true),
  maxAutomaticModificationRatio: z.number().min(0).max(1).default(0.45),
  applyTo: z.array(z.enum(["chapter", "short-fiction", "continuation", "revision"])).default([
    "chapter",
    "short-fiction",
    "continuation",
    "revision",
...
export type ProseQualityConfig = z.infer<typeof ProseQualityConfigSchema>;

export const LongFormMemoryConfigSchema = z.object({
  enabled: z.boolean().default(true),
  authority: z.literal("chapter-commit").default("chapter-commit"),
  strictPreflight: z.boolean().default(true),
  blockOnProjectionFailure: z.boolean().default(true),
  generateSequenceSummaries: z.boolean().default(true),
  sequenceSize: z.number().int().min(5).max(10).default(8),
  generateArcSummaries: z.boolean().default(true),
  retrieval: z.object({
    recentChapterCount: z.number().int().min(1).max(20).default(5),
    maxHistoricalEvents: z.number().int().min(0).max(100).default(20),
    maxRelatedSummaries: z.number().int().min(0).max(50).default(10),
    useFts: z.boolean().default(true),
    useEmbeddings: z.boolean().default(false),
    protectedTokenRatio: z.number().min(0).max(1).default(0.45),
    retrievedTokenRatio: z.number().min(0).max(1).default(0.30),
    compressedTokenRatio: z.number().min(0).max(1).default(0.25),
  }).default({
...
export const StorySpecConfigSchema = z.object({
  approvalMode: z.enum(["human", "automatic", "reviewer"]).default("human"),
  blockOnPlaceholders: z.boolean().default(true),
  requireReaderContract: z.boolean().default(true),
});

export type StorySpecConfig = z.infer<typeof StorySpecConfigSchema>;
...
export const WritingConfigSchema = z.object({
  ...
  storySpec: StorySpecConfigSchema.default({
    approvalMode: "human",
  ...
  longFormMemory: LongFormMemoryConfigSchema.default({
    enabled: true,
