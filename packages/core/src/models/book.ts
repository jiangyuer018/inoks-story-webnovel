...
export const BookConfigSchema = z.object({
  ...
  writing: z.object({
    ...
    proseQuality: z.object({
      ...
      enforcement: z.enum(["strict", "balanced", "report-only"]).optional(),
      autoRepair: z.boolean().optional(),
      maxRepairIterations: z.number().int().min(0).max(2).optional(),
      minimumScore: z.number().min(0).max(100).optional(),
      failOnUnresolvedBlocking: z.boolean().optional(),
      saveRejectedDraft: z.boolean().optional(),
      maxAutomaticModificationRatio: z.number().min(0).max(1).optional(),
      applyTo: z.array(z.enum(["chapter", "short-fiction", "continuation", "revision"])).optional(),
    }).optional(),
    longFormMemory: z.object({
      enabled: z.boolean().optional(),
      authority: z.literal("chapter-commit").optional(),
      strictPreflight: z.boolean().optional(),
      blockOnProjectionFailure: z.boolean().optional(),
      generateSequenceSummaries: z.boolean().optional(),
      sequenceSize: z.number().int().min(5).max(10).optional(),
      generateArcSummaries: z.boolean().optional(),
      retrieval: z.object({
        recentChapterCount: z.number().int().min(1).max(20).optional(),
        maxHistoricalEvents: z.number().int().min(0).max(100).optional(),
        maxRelatedSummaries: z.number().int().min(0).max(50).optional(),
        useFts: z.boolean().optional(),
        useEmbeddings: z.boolean().optional(),
        protectedTokenRatio: z.number().min(0).max(1).optional(),
        retrievedTokenRatio: z.number().min(0).max(1).optional(),
        compressedTokenRatio: z.number().min(0).max(1).optional(),
      }).optional(),
    }).optional(),
  }).optional(),
});
 ...
 * `writing.reviewMode` (inoks-story-webnovel.json); both unset falls back to "auto".
 */
 ...
 * `writing.revisionGate` (inoks-story-webnovel.json); both unset falls back to "strict".
 *
