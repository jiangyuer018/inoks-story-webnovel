...
export class BookWriteLockError extends Error {
  ...
  constructor(
    ...
    super(
      `Book "${bookId}" is locked by an active Inoks Story Webnovel write${lockData ? ` (${lockData})` : ""}. ` +
...
export class StateManager {
      ...
      return async () => {
        ...
        try {
          ...
          if ((error as NodeJS.ErrnoException | undefined)?.code !== "ENOENT") {
            console.warn(`[inoks-story] Failed to release book lock ${lockPath}: ${String(error)}`);
          }
        }
      };
    } catch (error) {
  ...
  async getBookLockInfo(bookId: string): Promise<BookLockInfo> {
    const lockPath = join(this.bookDir(bookId), ".write.lock");
    const lockKey = this.normalizeLockKey(lockPath);
    const inProcessOwner = processBookLocks.get(lockKey);
    try {
      const snapshot = await this.readLockSnapshot(lockPath);
      return {
        locked: true,
        lockPath,
        metadata: inProcessOwner?.metadata ?? snapshot.metadata,
        activeInProcess: Boolean(inProcessOwner),
        ownerProcessAlive: snapshot.metadata?.pid === process.pid
          ? Boolean(inProcessOwner)
          : snapshot.metadata?.pid !== undefined
            ? this.isProcessAlive(snapshot.metadata.pid)
            : undefined,
        stale: !inProcessOwner && this.isStaleLock(snapshot.metadata, snapshot.mtimeMs),
      };
    } catch (error) {
      if ((error as NodeJS.ErrnoException | undefined)?.code !== "ENOENT") throw error;
      return {
        locked: Boolean(inProcessOwner),
        lockPath,
        metadata: inProcessOwner?.metadata,
        activeInProcess: Boolean(inProcessOwner),
        ownerProcessAlive: Boolean(inProcessOwner),
        stale: false,
      };
    }
  }

  async forceReleaseBookLock(bookId: string, options: { readonly force?: boolean } = {}): Promise<BookLockInfo & { readonly removed: boolean }> {
    const info = await this.getBookLockInfo(bookId);
    if (info.activeInProcess && !options.force) {
      throw new Error(`Book "${bookId}" is still being written in this Inoks Story Webnovel process. Stop the task before forcing an unlock.`);
    }
    if (!info.locked) return { ...info, removed: false };
    if (!info.stale && !options.force) {
      throw new Error(`Book "${bookId}" has a live lock. Refusing to corrupt an active write.`);
    }
    if (info.activeInProcess) {
      const owner = processBookLocks.get(this.normalizeLockKey(info.lockPath));
      if (owner?.heartbeatTimer) clearInterval(owner.heartbeatTimer);
      await owner?.heartbeatTask;
      processBookLocks.delete(this.normalizeLockKey(info.lockPath));
    }
    await this.unlinkWithRetry(info.lockPath);
    return { ...info, locked: false, activeInProcess: false, ownerProcessAlive: false, stale: false, removed: true };
  }
  ...
  private startLockHeartbeat(lockPath: string, lockKey: string, owner: ProcessBookLock): void {
    ...
    owner.heartbeatTimer = setInterval(() => {
      ...
      const task = refresh()
        .catch((error) => {
          console.warn(`[inoks-story] Failed to refresh book lock ${lockPath}: ${String(error)}`);
        })
        .finally(() => {
  ...
  async loadProjectConfig(): Promise<Record<string, unknown>> {
    const configPath = join(this.projectRoot, "inoks-story-webnovel.json");
  ...
  async restoreState(bookId: string, chapterNumber: number): Promise<boolean> {
    const headPath = join(this.bookDir(bookId), ".inoks-story-webnovel", "story-system", "HEAD");
    let hasCommitAuthority = true;
    try {
      await stat(headPath);
    } catch (error) {
      if ((error as NodeJS.ErrnoException | undefined)?.code === "ENOENT") {
        hasCommitAuthority = false;
      } else {
        throw error;
      }
    }
    if (hasCommitAuthority) {
      throw new Error(
        `Legacy snapshot restore is disabled for "${bookId}" after ChapterCommit authority is enabled. `
        + "Create an amendment and replay projections instead.",
      );
    }
