...
export async function deleteLatestChapter(
  ...
  const headPath = join(bookDir, ".inoks-story-webnovel", "story-system", "HEAD");
  ...
  if (hasCommitAuthority) {
    throw new Error(
      `Deleting chapter history directly is disabled for "${bookId}" after ChapterCommit authority is enabled. `
      + "Record a retcon/manual-author-override amendment and replay projections instead.",
    );
  }
