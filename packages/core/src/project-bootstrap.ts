...
import { GLOBAL_ENV_PATH } from "./utils/llm-env.js";
...
export async function hasGlobalLLMConfig(): Promise<boolean> {
  try {
    const content = await readFile(GLOBAL_ENV_PATH, "utf-8");
    return content.includes("INOKS_STORY_LLM_API_KEY=") && !content.includes("your-api-key-here");
  } catch {
    return false;
  }
}
...
function buildProjectEnvTemplate(globalConfigured: boolean): string {
  if (globalConfigured) {
    return [
      ...
      "# Global config at ~/.inoks-story-webnovel/.env will be used by default.",
...
export async function initializeProjectDirectory(
  ...
  if (await exists(configPath)) {
    throw new Error(`inoks-story-webnovel.json already exists in ${projectDir}. Use a different directory or delete the existing project.`);
  }
