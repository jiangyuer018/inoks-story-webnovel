...
export function resolveServicePreset(service: string): ServicePreset | undefined {
  ...
  return {
    ...
    // piProvider 字段已从 InoksStoryEndpoint 移除（走 provider-to-pi-ai adapter），这里只保留 legacy fallback
...
function toModelInfo(inoksStoryModel: { id: string; maxOutput: number; contextWindowTokens: number }): ModelInfo {
  return {
    id: inoksStoryModel.id,
    name: inoksStoryModel.id,
    contextWindow: inoksStoryModel.contextWindowTokens,
    maxOutput: inoksStoryModel.maxOutput,
  };
}
 ...
 * - probe 失败或无 apiKey：fallback 到 provider.models（inoks-story bank）
 * - 不再做 INOKS_STORY_LLM_MODEL env 补丁（会污染跨 service 菜单；bank 已足够全）
 *
