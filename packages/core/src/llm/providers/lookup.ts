import type { InoksStoryModel } from "./types.js";
...
export function lookupModel(
  ...
  const matches: Array<{ model: InoksStoryModel; providerId: string }> = [];
...
export function listEnabledModels(serviceId: string): InoksStoryModel[] {
  const provider = getEndpoint(serviceId);
  if (!provider) return [];
  return provider.models.filter((m) => m.enabled !== false);
}

export function isActiveTextModel(model: InoksStoryModel): boolean {
...
export function listActiveTextModels(serviceId: string): InoksStoryModel[] {
  const provider = getEndpoint(serviceId);
  if (!provider) return [];
