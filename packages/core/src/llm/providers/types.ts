/**
 * inoks-story 自维护的 provider 定义。每个 provider 一个 .ts 文件，
 * 里面一个 InoksStoryEndpoint 对象（provider 元数据 + models 数组）。
 *
 * 数据冷启动自 lobe-chat/packages/model-bank，之后由 inoks-story 自管。
...
export interface InoksStoryModel {
...
export interface InoksStoryEndpoint {
  readonly id: string;
  ...
  readonly models: readonly InoksStoryModel[];
}
