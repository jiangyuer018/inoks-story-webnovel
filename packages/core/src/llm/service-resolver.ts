...
import type { InoksStoryEndpoint } from "./providers/types.js";
...
function resolveProviderCompat(
  provider: InoksStoryEndpoint | undefined,
  baseUrl: string,
...
export async function resolveServiceModel(
  ...
  if (!apiKey && !isApiKeyOptionalForEndpoint({ provider: preset?.providerFamily, baseUrl: effectiveBaseUrl })) {
    throw new Error(
      `API key not found for service "${service}". Add it in .inoks-story-webnovel/secrets.json or set the environment variable.`,
    );
  }
