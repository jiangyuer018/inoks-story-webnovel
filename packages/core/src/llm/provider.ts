...
const INOKS_STORY_USER_AGENT = "Inoks Story Webnovel/1.3.5";
...
function mergeUserAgent(headers?: Record<string, string>): Record<string, string> {
  return { "User-Agent": INOKS_STORY_USER_AGENT, ...(sanitizeHttpHeaders(headers) ?? {}) };
}
...
export function createLLMClient(config: LLMConfig): LLMClient {
  ...
  const inoksStoryProvider = getEndpoint(serviceName);
  const modelCard = lookupModel(serviceName, config.model);

  const piApi = resolvePiApi(serviceName, config.apiFormat, (inoksStoryProvider?.api ?? preset?.api) as PiApi) as PiApi;
  const baseUrl = config.baseUrl || inoksStoryProvider?.baseUrl || preset?.baseUrl || "";
  const extraHeaders = sanitizeHttpHeaders(config.headers ?? parseEnvHeaders());
  const compat = piApi === "openai-completions"
    ? resolveProviderCompat(inoksStoryProvider, baseUrl)
    : undefined;
  ...
  if (inoksStoryProvider?.id === "google") piProvider = "google";
  else if (inoksStoryProvider?.id === "zhipu") piProvider = "zai";
  else if (inoksStoryProvider?.id === "openrouter") piProvider = "openrouter";
  else if (inoksStoryProvider?.id === "githubCopilot") piProvider = "githubCopilot";
  else if (inoksStoryProvider?.id === "ollama") piProvider = "ollama";
  else if (inoksStoryProvider?.api === "anthropic-messages") piProvider = "anthropic";
...
function parseEnvHeaders(): Record<string, string> | undefined {
  const raw = process.env.INOKS_STORY_LLM_HEADERS;
...
export class ContextWindowExceededError extends Error {
  ...
  constructor(params: {
    ...
    super(
      `Inoks Story Webnovel context window guard: estimated input ${params.estimatedInputTokens} tokens + ` +
      ...
      `Inoks Story Webnovel will not truncate semantic text automatically.`,
    );
...
//
// inoks-story 让 writer/validator/architect 各自带 per-call 温度（0.1~1.5），
...
function clampTemperatureForModel(
  ...
  if (!warnedFixedTemperatureModels.has(model)) {
    ...
    console.warn(
      `[inoks-story] 模型 "${model}" API 要求 temperature=${locked}，已 clamp（原值 ${requested}）`,
    );
  }
  return locked;
}
...
function wrapLLMError(error: unknown, context?: { readonly baseUrl?: string; readonly model?: string; readonly service?: string }): Error {
  ...
  if (msg.includes("403")) {
    return new Error(
      ...
      `  3. 账户余额不足\n` +
      `  建议：用 inoks-story doctor 测试 API 连通性，或换一个不限制内容的 API 提供方${ctxLine}`,
    );
  }
  if (msg.includes("401")) {
    return new Error(
      `API 返回 401 (未授权)。请检查 .env 中的 INOKS_STORY_LLM_API_KEY 是否正确。${ctxLine}`,
    );
  }
  if (msg.includes("429")) {
    return new Error(
  ...
  if (
    ...
    return new Error(
      ...
      `  2. 网络不通或被防火墙拦截\n` +
      `  3. API 服务暂时不可用\n` +
      `  建议：检查 INOKS_STORY_LLM_BASE_URL 是否包含完整路径（如 /v1）`,
    );
  }
...
async function chatCompletionViaCustomAnthropicCompatible(
  ...
  const response = await fetchWithProxy(`${baseUrl.replace(/\/$/, "")}/messages`, {
    ...
    headers: sanitizeHttpHeaders({
      "User-Agent": INOKS_STORY_USER_AGENT,
      "x-api-key": apiKey,
...
/** Convert inoks-story LLMMessage[] to pi-ai Context. */
...
async function chatCompletionViaPiAi(
  ...
  if (!client.stream) {
    ...
    if (!content) {
      ...
      console.warn(`[inoks-story] LLM 非流式响应无文本内容 (${diag})`);
  ...
  if (!content) {
    ...
    console.warn(`[inoks-story] LLM 流式响应无文本内容 (${diag})`);
