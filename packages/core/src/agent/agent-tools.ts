...
const SuggestedActionParam = Type.Union([
  ...
  }, { description: "A model may describe an action as an object; Inoks Story Webnovel will normalize it to one short action string." }),
...
function proposedActionFallbackTitle(action: ProposeActionParamsType["action"], isZh: boolean): string {
  switch (action) {
    ...
    case "short_run":
      return isZh ? "生成 Inoks Story Webnovel Short" : "Generate Inoks Story Webnovel Short";
    case "play_start":
      return isZh ? "启动 Inoks Story Webnovel Play" : "Start Inoks Story Webnovel Play";
    case "generate_cover":
...
function proposedActionFallbackSummary(action: ProposeActionParamsType["action"], isZh: boolean): string {
  if (proposedActionTargetRoute(action)) {
    return isZh
      ...
      : "After confirmation, Inoks Story Webnovel will only open the existing Studio tool; it will not generate finished content directly.";
  }
  return isZh
    ? "确认后会切换到对应入口并执行这条需求。"
    : "After confirmation, Inoks Story Webnovel will switch to the matching surface and run this request.";
}
...
const ArchitectCreateSubAgentParams = Type.Object({
  ...
  bookId: Type.Optional(Type.String({
    description: "Optional new book ID. Usually omit it and let Inoks Story Webnovel derive the ID from title.",
  })),
...
export function createSubAgentTool(
  ...
  return {
    ...
    description: options.architectCreateOnly
      ? "Create a new long-form Inoks Story Webnovel book foundation. This confirmation turn can only call agent='architect'; writing chapters happens after the session is bound to the created book."
...
export function createResearchWebTool(projectRoot: string): AgentTool<typeof ResearchWebParams> {
  return {
    ...
    description:
      ...
      "Saves a Markdown report under .inoks-story-webnovel/research/. It is reference material only; it must not modify books, chapters, or truth files.",
    label: "Research Web",
    ...
    async execute(
      ...
      const reportDir = join(projectRoot, ".inoks-story-webnovel", "research");
...
const IngestMaterialParams = Type.Object({
  ...
  filePath: Type.Optional(Type.String({
    description: "Project-relative stored_path from the Uploaded Files block, e.g. .inoks-story-webnovel/uploads/session/file.pdf.",
  })),
...
export function createIngestMaterialTool(projectRoot: string): AgentTool<typeof IngestMaterialParams> {
  return {
    ...
    description:
      "Extract and archive a user-provided URL or uploaded file into .inoks-story-webnovel/materials as traceable Markdown. " +
...
export function createRetrieveMaterialTool(projectRoot: string): AgentTool<typeof RetrieveMaterialParams> {
  return {
    ...
    description:
      "Retrieve traceable snippets from previously ingested .inoks-story-webnovel/materials reference cards. " +
      "The agent supplies the semantic query; Inoks Story Webnovel returns evidence pointers. This must not mutate canon, chapters, scripts, or play state.",
...
const ImportChaptersParams = Type.Object({
  ...
  sourcePath: Type.String({
    description: "Local path of the chapter source: either the stored_path from the Uploaded Files block (project-relative, e.g. .inoks-story-webnovel/uploads/<session>/novel.txt) or an absolute path on this machine that the user provided. A directory imports each .md/.txt file as one chapter in filename order; a single file is split into chapters automatically by heading lines.",
  }),
...
export function createImportChaptersTool(
  ...
  return {
    ...
    description:
      "Import an existing novel's chapters from a local file or directory into an Inoks Story Webnovel book as real chapters (not reference material). " +
      "Inoks Story Webnovel reverse-engineers foundation/truth files from the imported text and replays every chapter to rebuild story state, so the book can be continued afterwards. " +
...
async function readResearchSearchConfig(projectRoot: string) {
  try {
    const raw = JSON.parse(await readFile(join(projectRoot, "inoks-story-webnovel.json"), "utf-8")) as Record<string, unknown>;
...
const ShortFictionRunParams = Type.Object({
  ...
  coverApiKeyEnv: Type.Optional(Type.String({
    description: "Optional env var containing the cover API key. Default INOKS_STORY_COVER_API_KEY.",
  })),
});
...
export function createTranslationCreateTool(
  ...
  return {
    ...
    description:
      "Create an Inoks Story Webnovel translation project from an EPUB/PDF/TXT/Markdown file. " +
...
export function createPlayStartTool(
  ...
  return {
    name: "play_start",
    description:
      "Start an interactive Inoks Story Webnovel Play world directly from chat. " +
...
export function createPlayEditTool(
  ...
  return {
    name: "play_edit",
    description:
      "Persistently edit the active Inoks Story Webnovel Play world card, visual contract, player persona, or entity/role cards without advancing time or narrating a turn. " +
...
export function createPlayStepTool(
  ...
  return {
    name: "play_step",
    description:
      "Advance the current Inoks Story Webnovel Play world by one player action. " +
...
export function createPlayReviseTool(
  ...
  return {
    name: "play_revise",
    description:
      "Regenerate, edit, or restore the latest Inoks Story Webnovel Play turn using saved turn checkpoints. " +
