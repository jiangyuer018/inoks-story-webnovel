...
describe("StateManager", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), "inoks-story-test-"));
  ...
  describe("snapshotState / restoreState", () => {
    ...
    it("refuses legacy snapshot restore after ChapterCommit authority is enabled", async () => {
      const headDir = join(manager.bookDir(bookId), ".inoks-story-webnovel", "story-system");
      await mkdir(headDir, { recursive: true });
      await writeFile(join(headDir, "HEAD"), "commit-1\n", "utf-8");

      await expect(manager.restoreState(bookId, 1)).rejects.toThrow(
        "Legacy snapshot restore is disabled",
      );
    });
  ...
  describe("rollbackToChapter", () => {
    ...
    it("refuses legacy rollback after ChapterCommit authority is enabled", async () => {
      await setupRollbackBook();
      const headDir = join(manager.bookDir(bookId), ".inoks-story-webnovel", "story-system");
      await mkdir(headDir, { recursive: true });
      await writeFile(join(headDir, "HEAD"), "commit-3\n", "utf-8");

      await expect(manager.rollbackToChapter(bookId, 1)).rejects.toThrow(
        "Legacy snapshot restore is disabled",
      );
      await expect(readFile(join(manager.bookDir(bookId), "chapters", "0003_Title_Three.md"), "utf-8"))
        .resolves.toContain("Content 3");
    });
