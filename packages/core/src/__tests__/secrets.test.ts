...
describe("secrets", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-secrets-"));
  });
  ...
  describe("loadSecrets", () => {
    it("returns empty when .inoks-story-webnovel/secrets.json does not exist", async () => {
  ...
  describe("saveSecrets", () => {
    it("creates .inoks-story-webnovel dir and writes secrets file", async () => {
      await saveSecrets(root, {
        services: { deepseek: { apiKey: "sk-deep" } },
      });
      const raw = await readFile(join(root, ".inoks-story-webnovel", "secrets.json"), "utf-8");
