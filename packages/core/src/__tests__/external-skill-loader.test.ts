...
describe("external skill loader", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-external-skills-"));
  });
  ...
  it("loads project-local skills from .inoks-story-webnovel/skills without explicit configuration", async () => {
    const skillDir = join(root, ".inoks-story-webnovel", "skills", "detective-play");
    await mkdir(skillDir, { recursive: true });
    await writeFile(
      join(skillDir, "SKILL.md"),
      [
  ...
  it("loads external skills from INOKS_STORY_SKILL_DIRS and reports bad paths without throwing", async () => {
    ...
    const loaded = await loadConfiguredCapabilitySkills({
      ...
      env: {
        INOKS_STORY_SKILL_DIRS: [externalRoot, join(root, "does-not-exist")].join(delimiter),
      },
    });
    const registry = createSkillRegistry({ skills: loaded.skills });
