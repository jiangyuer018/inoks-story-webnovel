...
`,
  ...
  it("resolves cover generation from project cover config and stored cover secret", async () => {
    const root = await mkdtemp(join(tmpdir(), "inoks-story-short-cover-"));
    try {
  ...
  it("generates a standalone cover artifact without running the short fiction pipeline", async () => {
    const root = await mkdtemp(join(tmpdir(), "inoks-story-cover-tool-"));
    const originalFetch = globalThis.fetch;
    const controller = new AbortController();
    process.env.INOKS_STORY_TEST_COVER_KEY = "sk-cover";
    try {
      ...
      const result = await generateShortFictionCover({
        ...
        coverApiKeyEnv: "INOKS_STORY_TEST_COVER_KEY",
      ...
      delete process.env.INOKS_STORY_TEST_COVER_KEY;
      await rm(root, { recursive: true, force: true });
    }
  });
});
