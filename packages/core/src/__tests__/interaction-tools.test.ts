...
describe("interaction tools", () => {
  beforeAll(async () => {
    projectRoot = await mkdtemp(join(tmpdir(), "inoks-story-core-interaction-tools-"));
  ...
  it("takes the book lock before deterministic text edit transactions", async () => {
    const root = await mkdtemp(join(tmpdir(), "inoks-story-core-interaction-lock-"));
    try {
      ...
      let releases = 0;
      const acquireBookLock = vi.fn(async (_bookId: string) => async () => {
        releases += 1;
      });
      const state = {
        ensureControlDocuments: vi.fn(async () => {}),
      ...
      const pipeline = {
        ...
        applyManualChapterAmendment: vi.fn(async (
          ...
          try {
            await writeFile(
              join(root, "books", bookId, "chapters", "0001_Start.md"),
              chapterDocument,
              "utf-8",
            );
            return { applied: true };
