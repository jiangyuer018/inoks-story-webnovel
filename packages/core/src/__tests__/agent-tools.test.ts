...
describe("agent deterministic writing tools", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-agent-tools-"));
  ...
  function amendmentPipeline() {
    return {
      applyManualChapterAmendment: vi.fn(async (
        bookId: string,
  ...
  it("patches chapter text through the deterministic edit controller", async () => {
    const pipeline = amendmentPipeline();
    const tool = createPatchChapterTextTool(pipeline as never, root, "harbor");
    ...
    expect(pipeline.applyManualChapterAmendment).toHaveBeenCalledOnce();
  });
  ...
  it("replaces whole chapter text through the deterministic edit controller", async () => {
    const pipeline = amendmentPipeline();
    const tool = createReplaceChapterTextTool(pipeline as never, root, "harbor");
