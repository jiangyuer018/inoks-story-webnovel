...
async function setupBook(params: {
  ...
  const root = await mkdtemp(join(tmpdir(), "inoks-story-chapter-delete-"));
...
describe("deleteLatestChapter", () => {
  ...
  it("refuses deletion before moving files when ChapterCommit authority is enabled", async () => {
    const { root, bookDir } = await setupBook({
      bookId: "commit-book",
      chapters: [
        { number: 1, title: "起风", content: "第一章。" },
    ...
    await writeFile(join(headDir, "HEAD"), "commit-2\n", "utf-8");

    const state = new StateManager(root);
    await expect(deleteLatestChapter(state, "commit-book"))
      .rejects.toThrow("Deleting chapter history directly is disabled");
