...
const ENV_KEYS = [
  "INOKS_STORY_LLM_SERVICE",
  "INOKS_STORY_LLM_PROVIDER",
  "INOKS_STORY_LLM_BASE_URL",
  "INOKS_STORY_LLM_MODEL",
  "INOKS_STORY_LLM_API_KEY",
  "INOKS_STORY_LLM_TEMPERATURE",
  "INOKS_STORY_LLM_THINKING_BUDGET",
  "INOKS_STORY_LLM_API_FORMAT",
  "INOKS_STORY_LLM_STREAM",
  "INOKS_STORY_LLM_EXTRA_top_p",
  "INOKS_STORY_DEFAULT_LANGUAGE",
] as const;
...
describe("loadProjectConfig local provider auth", () => {
  ...
  it("allows missing API keys for localhost OpenAI-compatible endpoints", async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-config-loader-local-"));
    for (const key of ENV_KEYS) {
  ...
  it("still requires API keys for remote hosted endpoints", async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-config-loader-remote-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
    ...
    await writeFile(join(root, ".env"), "", "utf-8");
    await expect(loadProjectConfig(root)).rejects.toThrow(/INOKS_STORY_LLM_API_KEY not set/i);
  });

  it("loads service-based config using defaultModel and project secrets", async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-config-loader-services-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
    ...
    await mkdir(join(root, ".inoks-story-webnovel"), { recursive: true });
    await writeFile(
      join(root, ".inoks-story-webnovel", "secrets.json"),
  ...
  it("derives provider/baseUrl from the MiniMax preset single source of truth", async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-config-loader-minimax-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
  ...
  it("loads custom service config using custom secret key and entry baseUrl", async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-config-loader-custom-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
  ...
  it("keeps Studio config active when llm.configSource is studio", async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-config-loader-studio-source-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
    ...
    await writeFile(join(root, ".env"), [
      ...
      "INOKS_STORY_LLM_BASE_URL=https://api-vip.codex-for.me/v1",
      "INOKS_STORY_LLM_MODEL=gpt-5.4",
      "INOKS_STORY_LLM_API_KEY=sk-env",
    ].join("\n"), "utf-8");
    await mkdir(join(root, ".inoks-story-webnovel"), { recursive: true });
    await writeFile(
      join(root, ".inoks-story-webnovel", "secrets.json"),
  ...
  it("does not mix stale top-level env-era model/baseUrl with selected Studio service", async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-config-loader-studio-stale-top-level-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }
    ...
    await writeFile(join(root, ".env"), [
      "INOKS_STORY_LLM_PROVIDER=custom",
      "INOKS_STORY_LLM_BASE_URL=https://api.moonshot.cn/v1",
      "INOKS_STORY_LLM_MODEL=kimi-k2.5",
      "INOKS_STORY_LLM_API_KEY=sk-env-moon",
    ].join("\n"), "utf-8");
    await mkdir(join(root, ".inoks-story-webnovel"), { recursive: true });
    await writeFile(
      join(root, ".inoks-story-webnovel", "secrets.json"),
  ...
  it("falls back to env when Studio config is still the empty bootstrap state", async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-config-loader-studio-bootstrap-"));
    for (const key of ENV_KEYS) {
      previousEnv.set(key, process.env[key]);
      process.env[key] = "";
    }

    process.env.INOKS_STORY_LLM_PROVIDER = "openai";
    process.env.INOKS_STORY_LLM_BASE_URL = "https://api-vip.codex-for.me/v1";
    process.env.INOKS_STORY_LLM_MODEL = "gpt-5.4";
    process.env.INOKS_STORY_LLM_API_KEY = "sk-env";
