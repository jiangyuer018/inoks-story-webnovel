...
async function createRunnerFixture(
  ...
  const root = await mkdtemp(join(tmpdir(), "inoks-story-runner-test-"));
...
async function migrateFixtureHistory(
  root: string,
  state: StateManager,
  bookId: string,
): Promise<void> {
  await migrateLegacyStorySystem({
    projectRoot: root,
...
describe("PipelineRunner", () => {
  ...
  it("initializes control documents during book creation", async () => {
    const root = await mkdtemp(join(tmpdir(), "inoks-story-init-book-test-"));
  ...
  it("applies creation-draft overrides while initializing a book", async () => {
    const root = await mkdtemp(join(tmpdir(), "inoks-story-init-book-overrides-"));
  ...
  it("cleans staged files when initBook fails before foundation is complete", async () => {
    const root = await mkdtemp(join(tmpdir(), "inoks-story-init-rollback-"));
    const runner = new PipelineRunner({
      client: {
        provider: "openai",
        apiFormat: "chat",
  ...
  it("does not misclassify generic runtime errors as sqlite-unavailable fallback", async () => {
    ...
    try {
      await expect(runner.writeDraft(bookId))
        .rejects.toThrow(/committed but required projection failed/);
      await expect(readFile(
        join(state.bookDir(bookId), ".inoks-story-webnovel", "story-system", "HEAD"),
        "utf-8",
      )).resolves.toMatch(/^commit-/);
  ...
  it("passes reduced control inputs into auditor and reviser in v2 mode", async () => {
    ...
    try {
      await expect(runner.writeNextChapter(bookId, 220))
        .rejects.toThrow(/ChapterCommit rejected/);
  ...
  it("degrades to state-degraded when state validation errors instead of aborting", async () => {
    ...
    await expect(runner.writeNextChapter(bookId))
      .rejects.toThrow(/ChapterCommit rejected/);
    expect(await state.loadChapterIndex(bookId)).toEqual([]);
  ...
  it("persists a state-degraded chapter without advancing truth files when settlement retry still contradicts the body", async () => {
    ...
    await expect(readdir(chaptersDir)).resolves.not.toContain("0001_Test_Chapter.md");
  ...
  it("repairs the latest state-degraded chapter from persisted body without rewriting it", async () => {
    ...
    ).repairChapterState(bookId, 1)).rejects.toThrow(/story repair/);
  ...
  it("syncs the latest edited chapter body back into truth files without requiring state-degraded status", async () => {
    ...
    ).resyncChapterArtifacts(bookId, 1)).rejects.toThrow(/audited ChapterAmendment/);
    expect(settleSpy).not.toHaveBeenCalled();
    await expect(readFile(join(storyDir, "current_state.md"), "utf-8")).resolves.toBe("stable state");
    await expect(readFile(join(storyDir, "pending_hooks.md"), "utf-8")).resolves.toBe("stable hooks");
  ...
  it("reports only resumed chapters in import results", async () => {
    ...
    await Promise.all([
      writeFile(join(state.bookDir(bookId), "chapters", "0001_One.md"), "# 第1章 One\n\n1111111111", "utf-8"),
      writeFile(join(state.bookDir(bookId), "chapters", "0002_Two.md"), "# 第2章 Two\n\n22222222222222222222", "utf-8"),
    ]);
    await migrateFixtureHistory(root, state, bookId);
  ...
  it("tracks imported English chapters using word counts instead of characters", async () => {
    ...
    await Promise.all([
      writeFile(join(state.bookDir(bookId), "chapters", "0001_Prelude.md"), "# Chapter 1: Prelude\n\nOne two three", "utf-8"),
      writeFile(join(state.bookDir(bookId), "chapters", "0002_Crossroads.md"), "# Chapter 2: Crossroads\n\nFour five", "utf-8"),
    ]);
    await migrateFixtureHistory(root, state, bookId);
  ...
  async function createRevisionGateFixture(revisionGate?: "strict" | "lenient" | "always") {
    ...
    await migrateFixtureHistory(fixture.root, fixture.state, fixture.bookId);
  ...
  it("never commits a manual revision with new continuity criticals", async () => {
    ...
    try {
      await expect(runner.reviseDraft(bookId, 1))
        .rejects.toThrow(/Revision ChapterCommit rejected/);
      const savedChapter = await readFile(join(chaptersDir, "0001_Test_Chapter.md"), "utf-8");
      const savedIndex = await state.loadChapterIndex(bookId);
