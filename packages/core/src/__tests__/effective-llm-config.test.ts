...
describe("resolveEffectiveLLMConfig", () => {
  ...
  async function writeProject(llm: Record<string, unknown>) {
    root = await mkdtemp(join(tmpdir(), "inoks-story-effective-llm-"));
  ...
  async function writeSecrets(services: Record<string, { apiKey: string }>) {
    ...
    await writeFile(join(root, ".inoks-story-webnovel", "secrets.json"), JSON.stringify({ services }, null, 2), "utf-8");
  }
  ...
  it("CLI consumer 允许 INOKS_STORY_LLM_SERVICE 切换服务，并从 provider bank 推导 baseUrl", async () => {
    await writeProject({
      configSource: "studio",
      service: "google",
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {},
        project: {
          INOKS_STORY_LLM_SERVICE: "moonshot",
  ...
  it("CLI consumer 兼容旧 env：没有 INOKS_STORY_LLM_SERVICE 时从 baseUrl 反推 service", async () => {
    await writeProject({
      configSource: "studio",
      service: "google",
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          INOKS_STORY_LLM_SERVICE: "moonshot",
          INOKS_STORY_LLM_MODEL: "kimi-k2.5",
          INOKS_STORY_LLM_API_KEY: "sk-global-moon",
        },
        project: {
          INOKS_STORY_LLM_SERVICE: "deepseek",
          INOKS_STORY_LLM_MODEL: "deepseek-chat",
          INOKS_STORY_LLM_API_KEY: "sk-project-deepseek",
        },
        process: {
          INOKS_STORY_LLM_SERVICE: "zhipu",
          INOKS_STORY_LLM_MODEL: "glm-4-flash",
          INOKS_STORY_LLM_API_KEY: "sk-process-zhipu",
        },
      },
      requireApiKey: true,
    });
  ...
  it("旧 configSource=env 保持 legacy-env 行为", async () => {
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          INOKS_STORY_LLM_PROVIDER: "custom",
          INOKS_STORY_LLM_BASE_URL: "https://api.example.com/v1",
          INOKS_STORY_LLM_MODEL: "legacy-model",
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          ...
          INOKS_STORY_LLM_BASE_URL: "https://api.moonshot.cn/v1",
          INOKS_STORY_LLM_MODEL: "kimi-k2.5",
          INOKS_STORY_LLM_API_KEY: "sk-moon",
        },
        project: {},
        process: {
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          ...
          INOKS_STORY_LLM_MODEL: "legacy-model",
          INOKS_STORY_LLM_API_KEY: "sk-env",
          INOKS_STORY_LLM_API_FORMAT: "chat",
          INOKS_STORY_LLM_STREAM: "true",
        },
        project: {},
        process: {},
      },
  ...
  it("保留旧 INOKS_STORY_LLM_EXTRA_* 和 INOKS_STORY_DEFAULT_LANGUAGE 行为", async () => {
    await writeProject({
      configSource: "env",
      provider: "openai",
      baseUrl: "https://api.example.com/v1",
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        ...
        project: {
          INOKS_STORY_LLM_API_KEY: "sk-env",
          INOKS_STORY_LLM_EXTRA_top_p: "0.9",
          INOKS_STORY_DEFAULT_LANGUAGE: "en",
        },
        process: {},
      },
    });
  ...
  it("CLI override 优先级高于 env", async () => {
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {},
        project: { INOKS_STORY_LLM_SERVICE: "google", INOKS_STORY_LLM_MODEL: "gemini-2.5-pro" },
        process: {},
      },
  ...
  it("拒绝不属于最终 service 的模型", async () => {
    ...
    await expect(resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {},
        project: { INOKS_STORY_LLM_MODEL: "kimi-k2.5" },
        process: {},
      },
  ...
  it("CLI env 显式指定 openrouter 未列出模型时也直接透传", async () => {
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {},
        project: { INOKS_STORY_LLM_MODEL: "z-ai/glm-4.7" },
        process: {},
      },
    });
  ...
  it("CLI env 指向 Ollama 时允许用户本地安装的动态模型", async () => {
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          INOKS_STORY_LLM_SERVICE: "ollama",
          INOKS_STORY_LLM_PROVIDER: "openai",
          INOKS_STORY_LLM_BASE_URL: "http://127.0.0.1:11434/v1",
          INOKS_STORY_LLM_MODEL: "qwen3.6:35b-a3b",
        },
        project: {},
        process: {},
      },
