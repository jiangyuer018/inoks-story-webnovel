...
export interface EditExecutionDeps {
  ...
  /**
   * Canonical chapter edits must delegate to the pipeline's audited
   * ChapterAmendment path. The edit controller never writes chapter files.
   */
  readonly applyChapterAmendment?: (
    bookId: string,
    chapterNumber: number,
    chapterDocument: string,
  ) => Promise<{ readonly applied: boolean; readonly skippedReason?: string }>;
}
...
const COMMIT_DERIVED_TRUTH_FILES = new Set([
  "current_state.md",
  "pending_hooks.md",
...
async function executeChapterReplace(
  ...
  if (!deps.applyChapterAmendment) {
    throw new Error("Chapter edits require the audited ChapterAmendment pipeline.");
  }
  const amendment = await deps.applyChapterAmendment(
    request.bookId,
    request.chapterNumber,
    fullText.endsWith("\n") ? fullText : `${fullText}\n`,
  );
  if (!amendment.applied) {
    throw new Error(amendment.skippedReason ?? `Chapter ${request.chapterNumber} amendment was not applied.`);
  }
  ...
  return {
    ...
    reviewRequired: false,
    summary: `Accepted chapter ${request.chapterNumber} replacement as an audited amendment.`,
  };
}
...
async function executeChapterLocalEdit(
  ...
  return {
    ...
    reviewRequired: false,
    summary: `Accepted chapter ${request.chapterNumber} patch as an audited amendment.`,
  };
}

export function replaceChapterTargetText(content: string, targetText: string, replacementText: string): string {
...
export async function executeEditTransaction(
  ...
  try {
    await access(join(root, ".inoks-story-webnovel", "story-system", "HEAD"));
  } catch (error) {
    if (isMissingDirectoryError(error)) {
  ...
  if (hasCommitAuthority && request.kind === "entity-rename") {
    throw new Error(
      "Entity rename cannot use the legacy bulk editor after ChapterCommit authority is enabled. "
      + "Create an entity-merge/entity-split/manual-author-override amendment.",
    );
  }
  if (hasCommitAuthority
    && request.kind === "truth-file-edit"
    && COMMIT_DERIVED_TRUTH_FILES.has(normalizeTruthFileName(request.fileName))) {
    throw new Error(
      `${normalizeTruthFileName(request.fileName)} is a ChapterCommit projection and is read-only. `
      + "Change canon through an amendment and replay projections.",
    );
  }
