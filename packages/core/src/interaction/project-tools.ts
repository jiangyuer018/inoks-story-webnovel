...
import { executeEditTransaction, replaceChapterTargetText } from "./edit-controller.js";
...
type PipelineLike = Pick<PipelineRunner, "writeNextChapter" | "reviseDraft"> & {
  readonly applyManualChapterAmendment?: PipelineRunner["applyManualChapterAmendment"];
  readonly initBook?: (
    book: BookConfig,
    options?: {
...
export function createInteractionToolsFromDeps(
  ...
  return {
            ...
            [
              {
                role: "system",
                content: [
                  "You are Inoks Story Webnovel inside the terminal workbench.",
    ...
    patchChapterText: async (bookId, chapterNumber, targetText, replacementText) => {
      if (!pipeline.applyManualChapterAmendment) {
        throw new Error("Audited ChapterAmendment support is unavailable in this pipeline.");
      }
      const current = await readChapterDocument(state.bookDir(bookId), chapterNumber);
      const updated = replaceChapterTargetText(current, targetText, replacementText);
      if (updated === current) {
        throw new Error(`Patch target was not found in chapter ${chapterNumber}.`);
      }
      const execution = await pipeline.applyManualChapterAmendment(
        bookId,
        chapterNumber,
        updated,
      );
      return {
        __interaction: {
          ...
          responseText: execution.applied
            ? `Chapter ${chapterNumber} edit accepted as an audited amendment.`
            : execution.skippedReason ?? "Chapter edit was not applied.",
        },
      };
    },
    replaceChapterText: async (bookId, chapterNumber, fullText) => {
      ...
      const execution = await pipeline.applyManualChapterAmendment(bookId, chapterNumber, fullText);
      return {
        __interaction: {
          activeChapterNumber: chapterNumber,
          responseText: execution.applied
            ? `Chapter ${chapterNumber} replacement accepted as an audited amendment.`
            : execution.skippedReason ?? "Chapter replacement was not applied.",
        },
      };
    },
    renameEntity: async (bookId, oldValue, newValue) => withBookMutationLock(state, bookId, async () => {
      if (await hasChapterCommitAuthority(state.bookDir(bookId))) {
        throw new Error(
          "Entity rename touches canon and cannot use the legacy bulk editor after ChapterCommit authority is enabled. Create an entity-merge/entity-split/manual-author-override amendment.",
        );
      }
    ...
    writeTruthFile: async (bookId, fileName, content) => withBookMutationLock(state, bookId, async () => {
      ...
      if (await hasChapterCommitAuthority(state.bookDir(bookId))
        && isDerivedRuntimeTruthFile(safeFileName)) {
        throw new Error(
          `${safeFileName} is a ChapterCommit projection and is read-only. Change canon through a chapter/amendment commit and replay projections.`,
        );
      }
...
async function readChapterDocument(bookDir: string, chapterNumber: number): Promise<string> {
  ...
  if (!file) throw new Error(`Chapter ${chapterNumber} not found`);
  return readFile(join(bookDir, "chapters", file), "utf-8");
}

async function hasChapterCommitAuthority(bookDir: string): Promise<boolean> {
  return access(join(bookDir, ".inoks-story-webnovel", "story-system", "HEAD")).then(() => true, () => false);
}

function isDerivedRuntimeTruthFile(fileName: string): boolean {
  return new Set([
    "current_state.md",
    "pending_hooks.md",
    "chapter_summaries.md",
