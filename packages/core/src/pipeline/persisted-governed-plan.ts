...
function renderPersistedPlanMarkdown(
  ...
  return [
    ...
    "",
    "<!-- INOKS_STORY_PLAN_MEMO_START -->",
    renderMemoMarkdown(memo),
    "<!-- INOKS_STORY_PLAN_MEMO_END -->",
    "",
    "## Intent",
...
function extractMarkedBlock(markdown: string, name: string): string | undefined {
  const match = markdown.match(new RegExp(`<!--\\s*INOKS_STORY_PLAN_${name}_START\\s*-->\\s*([\\s\\S]*?)\\s*<!--\\s*INOKS_STORY_PLAN_${name}_END\\s*-->`, "m"));
  return match?.[1]?.trim();
}
