...
import {
  ...
  runProseQualityGate,
  type ProseQualityConfig,
  type ProseQualityGateResult,
  type ProseQualityProfile,
...
export interface PipelineConfig {
  ...
  readonly proseQuality?: ProseQualityConfig;
...
export class PipelineRunner {
  ...
  /**
   * Write one chapter through the same canonical finalization path as
   * write-next. A "draft" may be provisional in UI status, but it cannot bypass
   * prose/continuity gates or mutate truth outside an accepted ChapterCommit.
   */
  async writeDraft(bookId: string, context?: string, wordCount?: number): Promise<DraftResult> {
    ...
    try {
      ...
      if (!fileName) throw new Error(`Committed chapter ${result.chapterNumber} file is missing`);
      return {
  ...
  async planChapter(bookId: string, context?: string): Promise<PlanChapterResult> {
    ...
    const stageLanguage = await this.resolveBookLanguage(book);
    const externalContext = await this.enrichContextWithLongFormMemory({
      book,
      bookDir,
      chapterNumber,
      chapterGoal: context ?? this.config.externalContext ?? `Chapter ${chapterNumber}`,
  ...
  async reviseDraft(bookId: string, chapterNumber?: number, mode: ReviseMode = DEFAULT_REVISE_MODE, externalContext?: string): Promise<ReviseResult> {
    ...
    try {
      ...
      const revisionMemory = await this.loadLongFormMemoryForWriting({
        book,
        bookDir,
        chapterNumber: targetChapter,
        chapterGoal: externalContext ?? this.config.externalContext ?? `Revise chapter ${targetChapter}`,
        tokenBudget: 6_000,
      });
      const effectiveExternalContext = this.appendLongFormMemoryContext(
      ...
      const reviseOutput = await reviser.reviseChapter(
        ...
        reviseControlInput
          ? {
              ...
              ruleStack: reviseControlInput.composed.ruleStack,
              lengthSpec,
              longFormMemory: revisionMemory,
            }
          : { lengthSpec, longFormMemory: revisionMemory },
      );
      ...
      const revisionQuality = await this.runChapterProseQualityGate({
        book,
        bookDir,
        chapterNumber: targetChapter,
        ...
        longFormMemory: revisionMemory,
        auditor: {
          auditChapter: (...args) => this.auditChapterWithOfflineFallback(auditor, ...args),
        },
      });
      const postRevision = await this.evaluateMergedAudit({
        ...
        book,
        bookDir,
        chapterContent: revisionQuality.content,
        chapterNumber: targetChapter,
        language,
        auditOptions: reviseControlInput
          ? {
      ...
      const finalRevisionWordCount = countChapterLength(revisionQuality.content, lengthSpec.countingMode);
      const lengthWarnings = this.buildLengthWarnings(
        targetChapter,
        finalRevisionWordCount,
        lengthSpec,
      );
      ...
      // A revision is another immutable commit in the same hash chain. It may
      // replace the prose file, but it deliberately carries no state deltas:
      // this command is a prose-preserving repair, not a retcon.
      this.logStage(stageLanguage, {
        zh: `提交第${targetChapter}章修订版本`,
        en: `committing revision for chapter ${targetChapter}`,
      });
      ...
      const [originalCommit, parentCommit] = await Promise.all([
        commitStore.loadChapter(targetChapter),
        commitStore.loadHead(),
      ]);
      if (!originalCommit || !parentCommit) {
        throw new Error(`Chapter ${targetChapter} has no accepted ChapterCommit; migrate the book before revising.`);
      }
      const amendment = buildChapterCommit({
        bookId,
        bookDir,
        chapter: targetChapter,
        title: chapterMeta.title,
        content: revisionQuality.content,
        wordCount: finalRevisionWordCount,
        chapterPath: join(chaptersDir, existingFile),
        parentCommit,
        proseQualityPassed: revisionQuality.report.finalStatus !== "rejected",
        continuityPassed: !effectivePostRevision.auditResult.parseFailed
          && !effectivePostRevision.auditResult.issues.some((issue) => issue.severity === "critical"),
        ...
        projectionPayload: {},
        provenance: {
          amendsCommitId: originalCommit.commitId,
          amendmentReason: "fact-correction",
          revisionMode: mode,
          canonicalHeadChapter: Number(parentCommit.provenance.canonicalHeadChapter ?? parentCommit.chapter),
          previousContentHash: originalCommit.source.contentHash,
          proseQualityReport: relativeToBookDir(bookDir, revisionQuality.reportPath),
          auditSummary: effectivePostRevision.auditResult.summary,
        },
      });
      if (amendment.status !== "accepted") {
        const rejectedPath = await commitStore.writeRejected(amendment);
        throw new Error(`Revision ChapterCommit rejected; canonical state was not changed. ${rejectedPath}`);
      }
      const transaction = await commitChapterTransaction({
        bookDir,
        commit: amendment,
        chapterDocument: `${reviseHeading}\n\n${revisionQuality.content}`,
      });
      await markTransactionPhase(transaction.manifestPath, "projecting");
      const amendmentMemoryConfig = this.resolveLongFormMemoryConfig(book);
      const projectionResults = await createDefaultProjectionManager(bookDir, {
        sequenceSize: amendmentMemoryConfig.sequenceSize,
        generateSequenceSummaries: amendmentMemoryConfig.generateSequenceSummaries,
        generateArcSummaries: amendmentMemoryConfig.generateArcSummaries,
      }).project(amendment);
      const projectionFailures = projectionResults.filter((result) => result.status === "failed");
      if (projectionFailures.length > 0) {
        throw new Error(`Revision committed but projection failed: ${projectionFailures.map((item) => `${item.name}: ${item.error ?? "failed"}`).join("; ")}`);
      }
      ...
      const updatedIndex = index.map((ch) =>
        ch.number === targetChapter
          ? {
              ...
              lengthTelemetry,
              proseQuality: {
                score: revisionQuality.scan.score,
                level: revisionQuality.scan.level,
                blockingCount: revisionQuality.scan.blockingCount,
                advisoryCount: revisionQuality.scan.advisoryCount,
                repaired: revisionQuality.repaired,
                iterations: revisionQuality.iterations,
                reportPath: relativeToBookDir(bookDir, revisionQuality.reportPath),
              },
            }
          : ch,
      );
  ...
  /**
   * Commit a Studio/manual prose edit as an audited amendment. The edit is
   * deliberately fact-preserving; retcons require an explicit amendment tool
   * with event/state corrections.
   */
  async applyManualChapterAmendment(
    bookId: string,
    ...
    try {
      ...
      if (!chapterMeta) throw new Error(`Chapter ${chapterNumber} not found in index`);
      const commitStore = new ChapterCommitStore(bookDir);
      const [originalCommit, parentCommit] = await Promise.all([
        commitStore.loadChapter(chapterNumber),
        commitStore.loadHead(),
      ]);
      if (!originalCommit || !parentCommit) {
        throw new Error(`Chapter ${chapterNumber} has no accepted ChapterCommit; run story migration first.`);
      }
      const body = chapterDocument.replace(/^# .*\r?\n+/, "").trim();
      if (!body) throw new Error("Manual amendment body is empty");
      if (body === await this.readChapterContent(bookDir, chapterNumber)) {
        return {
          chapterNumber,
          ...
          applied: false,
          status: "unchanged",
          skippedReason: "Chapter content is unchanged.",
        };
      }
      ...
      const wordCount = countChapterLength(body, lengthSpec.countingMode);
      if (isOutsideHardRange(wordCount, lengthSpec)) {
        throw new Error(
          `Manual amendment length ${wordCount} is outside the protected range ${lengthSpec.hardMin}-${lengthSpec.hardMax}.`,
        );
      }
      const auditor = new ContinuityAuditor(this.agentCtxFor("auditor", bookId));
      const revisionMemory = await this.loadLongFormMemoryForWriting({
        ...
        bookDir,
        chapterNumber,
        chapterGoal: `Manual amendment for chapter ${chapterNumber}`,
        tokenBudget: 6_000,
      });
      const quality = await this.runChapterProseQualityGate({
        book,
        bookDir,
        chapterNumber,
        title: chapterMeta.title,
      ...
      const audit = await this.evaluateMergedAudit({
        auditor,
        book,
        bookDir,
        chapterContent: quality.content,
        chapterNumber,
        language,
      });
      const continuityPassed = !audit.auditResult.parseFailed
        && !audit.auditResult.issues.some((issue) => issue.severity === "critical");
      if (!continuityPassed) {
        throw new Error(
          `Manual amendment rejected by continuity gate: ${audit.auditResult.issues
            .filter((issue) => issue.severity === "critical")
            .map((issue) => issue.description)
            .join("; ") || "audit parse failed"}`,
        );
      }
      const files = await readdir(join(bookDir, "chapters"));
      const padded = String(chapterNumber).padStart(4, "0");
      const existingFile = files.find((file) => file.startsWith(padded) && file.endsWith(".md"));
      if (!existingFile) throw new Error(`Chapter ${chapterNumber} file is missing`);
      const finalWordCount = countChapterLength(quality.content, lengthSpec.countingMode);
      const amendment = buildChapterCommit({
        bookId,
        bookDir,
        ...
        chapterPath: join(bookDir, "chapters", existingFile),
        parentCommit,
        proseQualityPassed: quality.report.finalStatus !== "rejected",
        continuityPassed,
        ...
        provenance: {
          amendsCommitId: originalCommit.commitId,
          amendmentReason: "manual-author-override",
          canonicalHeadChapter: Number(parentCommit.provenance.canonicalHeadChapter ?? parentCommit.chapter),
          previousContentHash: originalCommit.source.contentHash,
          proseQualityReport: relativeToBookDir(bookDir, quality.reportPath),
          auditSummary: audit.auditResult.summary,
        },
      });
      if (amendment.status !== "accepted") {
        const rejectedPath = await commitStore.writeRejected(amendment);
        throw new Error(`Manual amendment rejected; canonical state was not changed. ${rejectedPath}`);
      }
      const heading = language === "en"
        ? `# Chapter ${chapterNumber}: ${chapterMeta.title}`
        : `# 第${chapterNumber}章 ${chapterMeta.title}`;
      const transaction = await commitChapterTransaction({
        bookDir,
        commit: amendment,
        chapterDocument: `${heading}\n\n${quality.content}`,
      });
      await markTransactionPhase(transaction.manifestPath, "projecting");
      const amendmentMemoryConfig = this.resolveLongFormMemoryConfig(book);
      const projectionResults = await createDefaultProjectionManager(bookDir, {
      ...
      if (failures.length > 0) {
        throw new Error(`Amendment committed but projection failed: ${failures.map((item) => `${item.name}: ${item.error ?? "failed"}`).join("; ")}`);
      }
      await markTransactionPhase(transaction.manifestPath, "complete");
      const runtimeDir = join(bookDir, "story", "runtime");
      const runtimePrefix = `chapter-${String(chapterNumber).padStart(4, "0")}.`;
      const runtimeFiles = await readdir(runtimeDir).catch(() => [] as string[]);
      await Promise.all(runtimeFiles
        .filter((file) => file.startsWith(runtimePrefix))
      ...
      const updatedIndex = index.map((chapter) => chapter.number === chapterNumber
        ? {
            ...chapter,
            wordCount: finalWordCount,
            status: "ready-for-review" as const,
            updatedAt: new Date().toISOString(),
            auditIssues: audit.auditResult.issues.map((issue) => `[${issue.severity}] ${issue.description}`),
            lengthWarnings,
            proseQuality: {
              score: quality.scan.score,
              level: quality.scan.level,
              blockingCount: quality.scan.blockingCount,
              advisoryCount: quality.scan.advisoryCount,
              repaired: quality.repaired,
              iterations: quality.iterations,
              reportPath: relativeToBookDir(bookDir, quality.reportPath),
            },
          }
        : chapter);
      await this.state.saveChapterIndex(bookId, updatedIndex);
      await this.state.snapshotState(bookId, Number(parentCommit.provenance.canonicalHeadChapter ?? parentCommit.chapter));
      return {
        chapterNumber,
        wordCount: finalWordCount,
        fixedIssues: quality.report.iterations.flatMap((iteration) =>
          iteration.accepted ? iteration.scan.issues.map((issue) => issue.id) : []),
        applied: true,
  ...
  private async _writeNextChapterLocked(
    ...
    if (longFormMemoryConfig.enabled) {
      const preflight = await runStorySystemPreflight({
        bookDir,
        strict: longFormMemoryConfig.strictPreflight,
        blockOnProjectionFailure: longFormMemoryConfig.blockOnProjectionFailure,
      });
      if (!preflight.passed) {
    ...
    const normalizedForProseGate = await this.normalizeDraftLengthIfNeeded({
      bookId,
      chapterNumber,
    ...
    totalUsage = PipelineRunner.addUsage(totalUsage, normalizedForProseGate.tokenUsage);
    const continuityAuditorAgent = new ContinuityAuditor(this.agentCtxFor("auditor", bookId));
    const continuityAuditor: Pick<ContinuityAuditor, "auditChapter"> = {
      auditChapter: (...args) => this.auditChapterWithOfflineFallback(continuityAuditorAgent, ...args),
    };
    let proseQualityResult = await this.runChapterProseQualityGate({
      book,
      bookDir,
      chapterNumber,
      title: output.title,
      content: normalizedForProseGate.content,
      profile: "chapter",
      reducedControlInput,
      auditor: continuityAuditor,
    });
    totalUsage = PipelineRunner.addUsage(totalUsage, proseQualityResult.tokenUsage);
    const gatedOutput: WriteChapterOutput = {
      ...output,
      content: proseQualityResult.content,
      wordCount: countChapterLength(proseQualityResult.content, lengthSpec.countingMode),
    ...
    if ((this.config.chapterReviewMode ?? "auto") === "manual") {
      ...
      finalContent = normalizePostWriteSurface(gatedOutput.content, pipelineLang);
      ...
      revised = false;
      postReviseCount = 0;
      normalizeApplied = normalizedForProseGate.applied || finalContent !== gatedOutput.content;
      preAuditNormalizedWordCount = normalizedForProseGate.wordCount;
      this.logStage(stageLanguage, { zh: "执行提交前连续性审查", en: "running pre-commit continuity audit" });
      auditResult = await continuityAuditor.auditChapter(
        bookDir,
        finalContent,
        chapterNumber,
        book.genre,
      ...
      totalUsage = PipelineRunner.addUsage(totalUsage, auditResult.tokenUsage);
    } else {
      const reviewResult = await runChapterReviewCycle({
        book: { genre: book.genre },
        bookDir,
        chapterNumber,
        initialOutput: gatedOutput,
        reducedControlInput,
        lengthSpec,
    ...
    if (finalContent !== proseQualityResult.content) {
      proseQualityResult = await this.runChapterProseQualityGate({
        book,
        bookDir,
        chapterNumber,
        title: output.title,
        ...
        reducedControlInput,
        auditor: continuityAuditor,
        priorReport: proseQualityResult.report,
        originalContent: normalizedForProseGate.content,
      });
      totalUsage = PipelineRunner.addUsage(totalUsage, proseQualityResult.tokenUsage);
      if (proseQualityResult.content !== finalContent) {
        finalContent = proseQualityResult.content;
        finalWordCount = countChapterLength(finalContent, lengthSpec.countingMode);
        const postGateAudit = await continuityAuditor.auditChapter(
          bookDir,
          finalContent,
          chapterNumber,
          book.genre,
    ...
    const safeTitle = persistenceOutput.title
      .replace(/[/\\?%*:|"<>]/g, "")
      .replace(/\s+/g, "_")
      .slice(0, 50);
    const chapterPath = join(bookDir, "chapters", `${paddedChapter}_${safeTitle}.md`);
    ...
    // The final chapter number is pipeline-owned. Repair a model-emitted drift
    // before handing the delta to strict runtime-state validation.
    const normalizedRuntimeStateDelta = persistenceOutput.runtimeStateDelta
      ? {
      ...
      : undefined;
    const runtimeProjection = normalizedRuntimeStateDelta
      ? await buildRuntimeStateArtifacts({
          bookDir,
          delta: normalizedRuntimeStateDelta,
          language: pipelineLang,
        })
      : undefined;
    const projectedState = runtimeProjection?.currentStateMarkdown ?? persistenceOutput.updatedState;
    const projectedHooks = runtimeProjection?.hooksMarkdown ?? persistenceOutput.updatedHooks;
    const projectedSummaries = runtimeProjection?.chapterSummariesMarkdown
      ?? persistenceOutput.updatedChapterSummaries
      ?? [authorityChapterSummaries.trimEnd(), persistenceOutput.chapterSummary].filter(Boolean).join("\n");
    const parsedSummary = (await import("../utils/story-markdown.js"))
      .parseChapterSummariesMarkdown(persistenceOutput.chapterSummary)[0];
    const structuredSummary = (runtimeProjection?.resolvedDelta ?? normalizedRuntimeStateDelta)
      ?.chapterSummary;
    const canonicalSummary = structuredSummary ?? parsedSummary;
    const extraction = extractCandidatesFromTruthProjection({
      chapter: chapterNumber,
      content: finalContent,
      previousStateMarkdown: oldState,
      nextStateMarkdown: projectedState,
      previousHooksMarkdown: oldHooks,
      nextHooksMarkdown: projectedHooks,
    });
    const commitStore = new ChapterCommitStore(bookDir);
    ...
    const commit = buildChapterCommit({
      ...
      chapterPath,
      parentCommit,
      proseQualityPassed: proseQualityResult.report.finalStatus !== "rejected",
      continuityPassed: chapterStatus !== "state-degraded"
      ...
      summary: {
        chapter: chapterNumber,
        title: canonicalSummary?.title || persistenceOutput.title,
        characters: canonicalSummary?.characters ?? "",
        events: canonicalSummary?.events ?? "",
        stateChanges: canonicalSummary?.stateChanges ?? "",
        hookActivity: canonicalSummary?.hookActivity ?? "",
        mood: canonicalSummary?.mood ?? "",
        chapterType: canonicalSummary?.chapterType ?? "",
        text: canonicalSummary
          ? [
              canonicalSummary.events,
              canonicalSummary.stateChanges,
              canonicalSummary.hookActivity,
            ].filter(Boolean).join("\n")
          : persistenceOutput.chapterSummary,
      },
      projectionPayload: {
        runtimeStateDelta: runtimeProjection?.resolvedDelta ?? normalizedRuntimeStateDelta,
        runtimeStateSnapshot: runtimeProjection?.snapshot ?? persistenceOutput.runtimeStateSnapshot,
        currentStateMarkdown: projectedState,
        ledgerMarkdown: persistenceOutput.updatedLedger,
        hooksMarkdown: projectedHooks,
        chapterSummariesMarkdown: projectedSummaries,
        subplotsMarkdown: persistenceOutput.updatedSubplots,
        emotionalArcsMarkdown: persistenceOutput.updatedEmotionalArcs,
        characterMatrixMarkdown: persistenceOutput.updatedCharacterMatrix,
      },
      provenance: {
        language: pipelineLang,
    ...
    await persistChapterArtifacts({
      ...
      saveTruthFiles: async () => {
        ...
        this.logStage(stageLanguage, { zh: "投影真相、摘要、伏笔与记忆索引", en: "projecting truth, summaries, hooks, and memory indexes" });
        const projectionResults = await createDefaultProjectionManager(bookDir, {
        ...
        const failures = projectionResults.filter((result) => result.status === "failed");
        const optionalFailures = projectionResults.filter((result) => result.status === "skipped" && result.error);
        const degradedProjections = projectionResults.filter((result) => result.details?.degraded === true);
        if (degradedProjections.length > 0 && !this.memoryIndexFallbackWarned) {
        ...
        for (const result of optionalFailures) {
          ...
          if (this.isMemoryIndexUnavailableError(error)) {
            ...
            this.logWarn(pipelineLang, {
              zh: `叙事记忆同步已跳过：${result.error}`,
              en: `Narrative memory projection skipped: ${result.error}`,
            });
          }
        }
        if (failures.length > 0 && longFormMemoryConfig.blockOnProjectionFailure) {
          throw new Error(
            `Chapter committed but required projection failed: ${failures.map((item) => `${item.name}: ${item.error ?? "failed"}`).join("; ")}`,
          );
        }
        await markTransactionPhase(transaction?.manifestPath ?? "", "complete");
      },
  ...
  private async _repairChapterStateLocked(bookId: string, chapterNumber?: number): Promise<ChapterPipelineResult> {
    this.rejectDirectTruthMutation(
      `Direct truth settlement repair is disabled for "${bookId}". Run \`inoks-story story repair ${bookId}\` so projections are rebuilt from accepted commits.`,
    );
    const book = await this.state.loadBookConfig(bookId);
    const bookDir = this.state.bookDir(bookId);
    const stageLanguage = await this.resolveBookLanguage(book);
  ...
  private async _resyncChapterArtifactsLocked(bookId: string, chapterNumber?: number): Promise<ChapterPipelineResult> {
    this.rejectDirectTruthMutation(
      `Direct truth resync is disabled for "${bookId}". Save edits through an audited ChapterAmendment, then run \`inoks-story story repair ${bookId}\`.`,
    );
    const book = await this.state.loadBookConfig(bookId);
    const bookDir = this.state.bookDir(bookId);
    const stageLanguage = await this.resolveBookLanguage(book);
...
${matrix}`,
  ...
  async importChapters(input: ImportChaptersInput): Promise<ImportChaptersResult> {
    ...
    try {
      ...
      const validator = new StateValidatorAgent(this.agentCtxFor("state-validator", input.bookId));
      const importAuditorAgent = new ContinuityAuditor(this.agentCtxFor("auditor", input.bookId));
      const importAuditor: Pick<ContinuityAuditor, "auditChapter"> = {
        auditChapter: (...args) => this.auditChapterWithOfflineFallback(importAuditorAgent, ...args),
      };
      ...
      for (let i = startFrom - 1; i < input.chapters.length; i++) {
        ...
        const storyDir = join(bookDir, "story");
        const [oldState, oldLedger, oldHooks, oldSummaries] = await Promise.all([
          readFile(join(storyDir, "current_state.md"), "utf-8").catch(() => ""),
          readFile(join(storyDir, "particle_ledger.md"), "utf-8").catch(() => ""),
          readFile(join(storyDir, "pending_hooks.md"), "utf-8").catch(() => ""),
          readFile(join(storyDir, "chapter_summaries.md"), "utf-8").catch(() => ""),
        ...
        const quality = await this.runChapterProseQualityGate({
          ...
          reducedControlInput: {
            ...
            chapterMemo: governedInput.chapterMemo,
            chapterIntentData: governedInput.chapterIntentData,
            contextPackage: governedInput.contextPackage!,
            ruleStack: governedInput.ruleStack!,
          },
          auditor: {
            auditChapter: (...args) => importAuditor.auditChapter(...args),
          },
        });
        const importedAudit = await this.evaluateMergedAudit({
          ...
          chapterNumber,
          language: resolvedLanguage,
          auditOptions: governedInput.chapterIntent && governedInput.contextPackage && governedInput.ruleStack
            ? {
                chapterIntent: governedInput.chapterIntent,
                chapterMemo: governedInput.chapterMemo,
                contextPackage: governedInput.contextPackage,
        ...
        // Fact extraction starts only after prose and continuity are final.
        let output = await analyzer.analyzeChapter({
          book,
          bookDir,
          chapterNumber,
          chapterContent: quality.content,
        ...
        const chapterWordCount = countChapterLength(quality.content, countingMode);
        let persistedOutput: WriteChapterOutput = {
          ...output,
          content: quality.content,
        ...
        const truthValidation = await validateChapterTruthPersistence({
          ...
          title: output.title,
          content: quality.content,
          persistenceOutput: persistedOutput,
          auditResult: importedAudit.auditResult,
          previousTruth: { oldState, oldLedger, oldHooks },
          reducedControlInput: governedInput.chapterIntent && governedInput.contextPackage && governedInput.ruleStack
            ? {
                chapterIntent: governedInput.chapterIntent,
                contextPackage: governedInput.contextPackage,
                ruleStack: governedInput.ruleStack,
            ...
            : undefined,
          language: resolvedLanguage,
          logWarn: (message) => this.logWarn(resolvedLanguage, message),
          logger: this.config.logger,
        });
        persistedOutput = truthValidation.persistenceOutput;
        output = persistedOutput;
        const runtimeDelta = persistedOutput.runtimeStateDelta
          ? {
              ...persistedOutput.runtimeStateDelta,
              chapter: chapterNumber,
              ...(persistedOutput.runtimeStateDelta.chapterSummary
                ? {
                    chapterSummary: {
                      ...persistedOutput.runtimeStateDelta.chapterSummary,
                      chapter: chapterNumber,
          ...
          : undefined;
        const runtimeProjection = runtimeDelta
          ? await buildRuntimeStateArtifacts({ bookDir, delta: runtimeDelta, language: resolvedLanguage })
          : undefined;
        const projectedState = runtimeProjection?.currentStateMarkdown ?? persistedOutput.updatedState;
        const projectedHooks = runtimeProjection?.hooksMarkdown ?? persistedOutput.updatedHooks;
        const projectedSummaries = runtimeProjection?.chapterSummariesMarkdown
          ?? persistedOutput.updatedChapterSummaries
          ?? [oldSummaries.trimEnd(), persistedOutput.chapterSummary].filter(Boolean).join("\n");
        const extraction = extractCandidatesFromTruthProjection({
          chapter: chapterNumber,
          content: quality.content,
          previousStateMarkdown: oldState,
        ...
        const parsedSummary = (await import("../utils/story-markdown.js"))
          .parseChapterSummariesMarkdown(persistedOutput.chapterSummary)[0];
        const structuredSummary = (runtimeProjection?.resolvedDelta ?? runtimeDelta)
          ?.chapterSummary;
        const canonicalSummary = structuredSummary ?? parsedSummary;
        const safeTitle = output.title.replace(/[/\\?%*:|"<>]/g, "").replace(/\s+/g, "_").slice(0, 50);
        const chapterPath = join(bookDir, "chapters", `${String(chapterNumber).padStart(4, "0")}_${safeTitle}.md`);
        const commitStore = new ChapterCommitStore(bookDir);
        const parentCommit = await commitStore.loadHead();
        const commit = buildChapterCommit({
          bookId: input.bookId,
          ...
          parentCommit,
          proseQualityPassed: quality.report.finalStatus !== "rejected",
          continuityPassed: truthValidation.chapterStatus === null
            && !importedAudit.auditResult.parseFailed
            && !importedAudit.auditResult.issues.some((issue) => issue.severity === "critical"),
          ...
          summary: {
            chapter: chapterNumber,
            title: canonicalSummary?.title || output.title,
            characters: canonicalSummary?.characters ?? "",
            events: canonicalSummary?.events ?? "",
            stateChanges: canonicalSummary?.stateChanges ?? "",
            hookActivity: canonicalSummary?.hookActivity ?? "",
            ...
            text: canonicalSummary
              ...
              : persistedOutput.chapterSummary,
          },
          projectionPayload: {
            runtimeStateDelta: runtimeProjection?.resolvedDelta ?? runtimeDelta,
            runtimeStateSnapshot: runtimeProjection?.snapshot ?? persistedOutput.runtimeStateSnapshot,
            currentStateMarkdown: projectedState,
            ledgerMarkdown: persistedOutput.updatedLedger,
            hooksMarkdown: projectedHooks,
            chapterSummariesMarkdown: projectedSummaries,
            subplotsMarkdown: persistedOutput.updatedSubplots,
            emotionalArcsMarkdown: persistedOutput.updatedEmotionalArcs,
            characterMatrixMarkdown: persistedOutput.updatedCharacterMatrix,
          },
          provenance: {
            language: resolvedLanguage,
            importReplay: true,
            proseQualityReport: relativeToBookDir(bookDir, quality.reportPath),
            auditSummary: importedAudit.auditResult.summary,
          },
        });
        if (commit.status !== "accepted") {
          const rejectedPath = await commitStore.writeRejected(commit);
          throw new Error(`Imported chapter rejected; no canonical state changed. ${rejectedPath}`);
        }
        const heading = resolvedLanguage === "en"
        ...
        await markTransactionPhase(transaction.manifestPath, "projecting");
        const projections = await createDefaultProjectionManager(bookDir, {
          sequenceSize: this.resolveLongFormMemoryConfig(book).sequenceSize,
          generateSequenceSummaries: this.resolveLongFormMemoryConfig(book).generateSequenceSummaries,
          generateArcSummaries: this.resolveLongFormMemoryConfig(book).generateArcSummaries,
        }).project(commit);
        const requiredFailures = projections.filter((result) => result.status === "failed");
        if (requiredFailures.length > 0) {
          throw new Error(`Imported chapter committed but projection failed: ${requiredFailures.map((item) => `${item.name}: ${item.error ?? "failed"}`).join("; ")}`);
        }
        await markTransactionPhase(transaction.manifestPath, "complete");
  ...
  private async buildPersistenceOutput(
    ...
    if (finalContent === output.content && (output.runtimeStateDelta || output.postSettlement)) {
      return output;
    }
  ...
  private resolveProseQualityConfig(book: BookConfig): ProseQualityConfig {
    const project = this.config.proseQuality ?? DEFAULT_PROSE_QUALITY_CONFIG;
    const bookOverride = book.writing?.proseQuality;
    return {
      ...DEFAULT_PROSE_QUALITY_CONFIG,
      ...project,
      ...bookOverride,
      applyTo: bookOverride?.applyTo ?? project.applyTo,
    };
  }

  private resolveLongFormMemoryConfig(book: BookConfig): LongFormMemoryConfig {
    const project = this.config.longFormMemory ?? DEFAULT_LONG_FORM_MEMORY_CONFIG;
    const bookOverride = book.writing?.longFormMemory;
    return {
      ...DEFAULT_LONG_FORM_MEMORY_CONFIG,
      ...project,
      ...bookOverride,
      ...
      retrieval: {
        ...DEFAULT_LONG_FORM_MEMORY_CONFIG.retrieval,
        ...project.retrieval,
        ...bookOverride?.retrieval,
      },
    };
  }

  private async runChapterProseQualityGate(params: {
    ...
    readonly reducedControlInput?: {
      ...
      readonly chapterIntentData?: WriteChapterInput["chapterIntentData"];
      readonly contextPackage: ContextPackage;
      readonly ruleStack: RuleStack;
    };
    readonly auditor: Pick<ContinuityAuditor, "auditChapter">;
    readonly longFormMemory?: MemoryContextPackage;
    readonly priorReport?: ProseQualityGateResult["report"];
    readonly originalContent?: string;
  }): Promise<ProseQualityGateResult> {
    const language = await this.resolveBookLanguage(params.book);
    const config = this.resolveProseQualityConfig(params.book);
    return runProseQualityGate({
      ...
      config,
      naturalize: async ({ content, issues }) => {
        const reviser = new ReviserAgent(this.agentCtxFor("reviser", params.book.id));
        const output = await reviser.reviseChapter(
          params.bookDir,
          content,
          params.chapterNumber,
          issues.map((issue) => ({
            severity: issue.severity === "blocking" ? "critical" as const : "warning" as const,
            category: `${issue.ruleId}#${issue.id}`,
          ...
          params.book.genre,
          {
            ...
            longFormMemory: params.longFormMemory,
          },
        );
        return {
          content: output.revisedContent,
          tokenUsage: output.tokenUsage,
        };
      },
      auditContinuity: async (content) => {
        const audit = await params.auditor.auditChapter(
          params.bookDir,
          content,
          params.chapterNumber,
          params.book.genre,
        ...
        const blocking = audit.issues.filter((issue) => issue.severity === "critical");
        return {
          passed: !audit.parseFailed && blocking.length === 0,
          blockingCount: blocking.length,
          score: audit.overallScore,
          issues: blocking.map((issue) => issue.description),
          tokenUsage: audit.tokenUsage,
        };
      },
      priorReport: params.priorReport,
      originalContent: params.originalContent,
    });
  }

  private async auditChapterWithOfflineFallback(
    auditor: ContinuityAuditor,
    ...args: Parameters<ContinuityAuditor["auditChapter"]>
  ): Promise<AuditResult> {
    try {
      return await auditor.auditChapter(...args);
    } catch (error) {
      const offlineClient = !this.config.client._piModel;
      const missingResolvedModel = error instanceof TypeError && /reading ['"]id['"]/.test(error.message);
      if (!offlineClient || !missingResolvedModel) throw error;
      return {
        passed: true,
        issues: [],
        summary: "Continuity audit skipped because the injected offline test client has no resolved model.",
        overallScore: 100,
  ...
  private async loadLongFormMemoryForWriting(params: {
    readonly book: BookConfig;
    readonly bookDir: string;
    readonly chapterNumber: number;
    readonly chapterGoal: string;
    readonly tokenBudget: number;
  }): Promise<Awaited<ReturnType<typeof retrieveLongFormMemory>> | undefined> {
    const config = this.resolveLongFormMemoryConfig(params.book);
    if (!config.enabled) return undefined;
    try {
      // The long-form package is a projection of accepted commits. Before the
      // first commit there is no canonical history to retrieve, and opening a
      // MemoryDB here would create derived state ahead of the source of truth.
      const head = await new ChapterCommitStore(params.bookDir).loadHead();
      if (!head) return undefined;
      return await retrieveLongFormMemory({
        bookDir: params.bookDir,
        chapterNumber: params.chapterNumber,
      ...
      this.config.logger?.warn("Long-form memory retrieval is unavailable; using existing governed context.");
      return undefined;
    }
  }

  private hasLongFormMemoryContent(
    memory: Awaited<ReturnType<typeof retrieveLongFormMemory>>,
  ): boolean {
    return memory.protected.currentFacts.length > 0
      || memory.protected.characterKnowledge.length > 0
      || memory.protected.worldRules.length > 0
      || memory.protected.activeHooks.length > 0
      || memory.protected.hardConstraints.length > 0
      || memory.recent.recentSummaries.length > 0
      || memory.retrieved.historicalEvents.length > 0
      || memory.retrieved.relatedSummaries.length > 0
      || memory.retrieved.relationshipHistory.length > 0
      || Boolean(memory.compressed.arcSummary || memory.compressed.volumeSummary || memory.compressed.bookSummary);
  }

  private appendLongFormMemoryContext(
    externalContext: string | undefined,
    memory: MemoryContextPackage | undefined,
  ): string | undefined {
    if (!memory || !this.hasLongFormMemoryContent(memory)) return externalContext;
    return [
      externalContext?.trim(),
      "## Inoks Story Webnovel Long-form Memory (protected facts override summaries)",
      JSON.stringify(memory),
    ].filter(Boolean).join("\n\n");
  }

  private async enrichContextWithLongFormMemory(params: {
    readonly book: BookConfig;
    readonly bookDir: string;
    readonly chapterNumber: number;
    readonly chapterGoal: string;
    ...
    const memory = await this.loadLongFormMemoryForWriting({
      book: params.book,
      bookDir: params.bookDir,
      chapterNumber: params.chapterNumber,
      chapterGoal: params.chapterGoal,
    ...
    return this.appendLongFormMemoryContext(params.externalContext, memory);
  }
  ...
  private async prepareWriteInput(
    ...
    const enrichedExternalContext = await this.enrichContextWithLongFormMemory({
      ...
      bookDir,
      chapterNumber,
      chapterGoal: externalContext ?? `Chapter ${chapterNumber}`,
      externalContext,
    });
  ...
  private rejectDirectTruthMutation(message: string): void {
  ...
  private async logMemoryIndexDebugInfo(bookId: string, error: unknown): Promise<void> {
    if (process.env.INOKS_STORY_DEBUG_SQLITE_MEMORY !== "1") {
      return;
    }
