...
import {
  ...
  PROSE_NATURALIZER_SYSTEM_PROMPT,
  runProseQualityGate,
...
export interface ShortFictionRunOptions {
  ...
  readonly proseQuality?: Partial<ProseQualityConfig>;
}
...
async function produceShort(
  ...
  try {
    ...
    options.onProgress?.("Running mandatory prose quality gate...");
    finalDraft = await qualityGateShortDraft({
      options,
      root,
      baseDir,
      draft: finalDraft,
...
async function qualityGateShortDraft(params: {
  ...
  const config: ProseQualityConfig = {
    ...DEFAULT_PROSE_QUALITY_CONFIG,
    ...params.options.proseQuality,
    applyTo: params.options.proseQuality?.applyTo ?? DEFAULT_PROSE_QUALITY_CONFIG.applyTo,
  };
  const bookDir = safeChildPath(params.root, params.baseDir);
  const countingMode = resolveLengthCountingMode(params.language);
  const chapters = [];
  for (const chapter of params.draft.chapters) {
    const gate = await runProseQualityGate({
      ...
      config,
      naturalize: async ({ content, issues, iteration }) => {
        const response = await chatCompletion(
          params.options.runtimes.revise.client,
          params.options.runtimes.revise.model,
          [
            { role: "system", content: PROSE_NATURALIZER_SYSTEM_PROMPT },
            {
              role: "user",
              content: [
                `短篇章节：${chapter.number}/${params.chapterCount}`,
                `修订轮次：${iteration}`,
                "仅处理以下结构化问题：",
          ...
          {
            temperature: 0.2,
            maxTokens: Math.max(2048, Math.ceil(content.length * 1.8)),
            onStreamProgress: params.options.runtimes.revise.onStreamProgress,
            signal: params.options.signal,
          },
        );
        return { content: response.content.trim(), tokenUsage: response.usage };
      },
    });
    chapters.push({
      ...chapter,
      content: gate.content,
      charCount: countChapterLength(gate.content, countingMode),
    });
  }
  const structuredDraft: ShortFictionBatchDraft = {
    ...params.draft,
    chapters,
    rawContent: "",
  };
  const gatedDraft: ShortFictionBatchDraft = {
    ...structuredDraft,
    rawContent: renderShortFictionDraftMarkdown(structuredDraft, params.language),
  };
  validateShortFictionDraftForFinal(gatedDraft, { expectedChapters: params.chapterCount });
  return gatedDraft;
}
...
async function generateCoverImageArtifact(input: {
  ...
  const size = input.coverSize || process.env.INOKS_STORY_COVER_SIZE || "1024x1360";
...
export async function resolveCoverGenerationRequest(input: {
  ...
  if (input.coverEndpoint || input.coverBaseUrl || process.env.INOKS_STORY_COVER_ENDPOINT || process.env.INOKS_STORY_COVER_BASE_URL) {
    const endpoint = resolveCoverEndpoint(input.coverEndpoint, input.coverBaseUrl);
    const baseUrl = input.coverBaseUrl || process.env.INOKS_STORY_COVER_BASE_URL || endpoint
      .replace(/\/responses\/?$/u, "")
    ...
    return {
      ...
      baseUrl,
      endpoint,
      model: input.coverModel || process.env.INOKS_STORY_COVER_MODEL || "gpt-image-2",
      apiKey: resolveCoverApiKey(input.coverApiKeyEnv || "INOKS_STORY_COVER_API_KEY"),
    };
  }

  const projectCover = await readProjectCoverConfig(input.root);
  if (!projectCover) {
    throw new Error("cover endpoint is required. Configure cover generation in Studio or set INOKS_STORY_COVER_BASE_URL.");
  }
...
async function readProjectCoverConfig(root: string): Promise<{ readonly service: string; readonly model?: string } | undefined> {
  try {
    const raw = await readFile(join(root, "inoks-story-webnovel.json"), "utf-8");
...
function resolveCoverEndpoint(coverEndpoint?: string, coverBaseUrl?: string): string {
  const endpoint = coverEndpoint || process.env.INOKS_STORY_COVER_ENDPOINT;
  if (endpoint) return endpoint;
  const baseUrl = coverBaseUrl || process.env.INOKS_STORY_COVER_BASE_URL;
  if (!baseUrl) {
    throw new Error("cover endpoint is required. Set INOKS_STORY_COVER_BASE_URL or disable cover generation.");
  }
