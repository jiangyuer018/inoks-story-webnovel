...
export class ConsolidatorAgent extends BaseAgent {
  ...
  async consolidate(bookDir: string): Promise<ConsolidationResult> {
    ...
    const usesCommitAuthority = await access(
      join(bookDir, ".inoks-story-webnovel", "story-system", "HEAD"),
    ).then(() => true, () => false);
    if (usesCommitAuthority) {
      const { ChapterCommitStore, repairStorySystem } = await import("../story-system/index.js");
      const repair = await repairStorySystem(bookDir);
      const failures = repair.results.filter((result) => result.status === "failed");
      if (failures.length > 0) {
        throw new Error(
          `ChapterCommit projection replay failed: ${failures
      ...
      return {
        volumeSummaries: await readFile(
          join(storyDir, "summaries", "volume-summary.md"),
          "utf-8",
        ).catch(() => ""),
        archivedVolumes: 0,
        retainedChapters: new Set(commits.map((commit) => commit.chapter)).size,
        promotedHookCount: 0,
      };
    }
