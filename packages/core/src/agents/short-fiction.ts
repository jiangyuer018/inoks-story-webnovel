...
export class ShortFictionWriterAgent extends BaseAgent {
  ...
  async writeDraft(input: ShortFictionDraftInput): Promise<ShortFictionBatchDraft> {
    const response = await retryShortFictionCall(() =>
      this.chat([
        { role: "system", content: shortFictionWriterSystemPrompt(input.language) },
...
function shortFictionWriterSystemPrompt(language: ShortFictionLanguage | undefined): string {
  const resolved = language ?? "zh";
  return `${buildShortFictionWriterSystemPrompt(language)}\n\n${buildCoreProseQualityConstraints(resolved)}`;
}
