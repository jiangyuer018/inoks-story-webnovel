...
async function applyContextBudgetIfNeeded(params: {
  ...
  if (protectedTokens > availableInputTokens) {
    ...
    throw new Error(
      ...
      "Inoks Story Webnovel will not compress protected author intent, current focus, hard state, or active hook evidence.",
    );
  }
...
export class ComposerAgent extends BaseAgent {
  ...
  async selectOutlineSections(request: OutlineSectionSelectionRequest): Promise<ReadonlyArray<string>> {
    ...
    const system = isEn
      ? [
          "You are Inoks Story Webnovel's semantic outline-section selector.",
      ...
      : [
          "你是 Inoks Story Webnovel 的语义大纲选段器。",
  ...
  async compileCompressibleContext(request: CompressibleContextCompileRequest): Promise<string> {
    ...
    const system = isEn
      ? [
          "You are Inoks Story Webnovel's semantic context compiler.",
      ...
      : [
          "你是 Inoks Story Webnovel 的语义上下文编译器。",
