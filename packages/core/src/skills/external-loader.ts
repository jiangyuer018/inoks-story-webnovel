...
function configuredSkillDirs(input: LoadConfiguredCapabilitySkillsInput): ConfiguredSkillDir[] {
  ...
  const envDirs = (env.INOKS_STORY_SKILL_DIRS ?? "")
    .split(delimiter)
    .map((value) => value.trim())
    .filter(Boolean);
  const userRoot = input.userRoot ?? join(homedir(), ".inoks-story-webnovel");
  return [
    { path: join(input.projectRoot, ".inoks-story-webnovel", "skills"), explicit: false },
