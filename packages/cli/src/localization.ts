import { formatLengthCount, resolveLengthCountingMode } from "@inoks-story-webnovel/core";
...
export function resolveCliLanguage(
  ...
  const requested = normalizeCliLanguageTag(env.INOKS_STORY_LOCALE);
  if (requested) {
    return requested;
  }
...
export function formatBookCreateNextStep(language: CliLanguage, bookId: string): string {
  return localize(language, {
    zh: `下一步：inoks-story write next ${bookId}`,
    en: `Next: inoks-story write next ${bookId}`,
  });
}
...
export function formatImportChaptersComplete(
  ...
  return [
    ...
    "",
    localize(language, {
      zh: `运行 "inoks-story write next ${result.continueBookId}" 继续写作。`,
      en: `Run "inoks-story write next ${result.continueBookId}" to continue writing.`,
    }),
  ];
}
...
export function formatDoctorHintBaseUrl(language: CliLanguage): string {
  return localize(language, {
    zh: "baseUrl 可能不正确，检查 INOKS_STORY_LLM_BASE_URL 是否包含完整路径（如 /v1）",
    en: "The baseUrl may be wrong. Check that INOKS_STORY_LLM_BASE_URL includes the full path (e.g. /v1).",
  });
}
...
export function formatDoctorHintModelName(language: CliLanguage): string {
  return localize(language, {
    zh: "检查模型名称是否正确（INOKS_STORY_LLM_MODEL）",
    en: "Check that the model name is correct (INOKS_STORY_LLM_MODEL).",
  });
}

export function formatDoctorHintInvalidApiKey(language: CliLanguage): string {
  return localize(language, {
    zh: "API Key 无效，检查 INOKS_STORY_LLM_API_KEY",
    en: "The API key is invalid. Check INOKS_STORY_LLM_API_KEY.",
  });
}
...
export function formatFanficCanonMissingError(): string {
  return "No fanfic canon found for this book. Create one with `inoks-story fanfic init`（该书没有同人正典文件，用 inoks-story fanfic init 创建同人书）";
}
...
export function formatBookBackupCreated(language: CliLanguage, bookId: string, backupId: string): string {
  return localize(language, {
    zh: `已备份 ${bookId} → .inoks-story-webnovel/backups/${bookId}/${backupId}/`,
    en: `Backed up ${bookId} → .inoks-story-webnovel/backups/${bookId}/${backupId}/`,
  });
}

export function formatBookBackupListEmpty(language: CliLanguage, bookId: string): string {
  return localize(language, {
    zh: `${bookId} 还没有备份。用 inoks-story book backup ${bookId} 创建一份。`,
    en: `No backups for ${bookId} yet. Create one with: inoks-story book backup ${bookId}`,
  });
}
