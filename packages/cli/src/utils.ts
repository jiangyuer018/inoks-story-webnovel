...
export function buildPipelineConfig(
  ...
  const logger = hasLogging ? createLogger({ tag: "inoks-story", sinks }) : undefined;
...
export async function resolveBookId(
  ...
  if (books.length === 0) {
    throw new Error(
      "No books found. Create one first:\n  inoks-story book create --title '...' --genre xuanhuan",
    );
  }
  if (books.length === 1) {
    return books[0]!;
