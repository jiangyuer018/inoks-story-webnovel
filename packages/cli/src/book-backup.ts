...
/**
 * Whole-book backups live OUTSIDE books/ (at .inoks-story-webnovel/backups/<bookId>/<backupId>/),
 * so a backup never recursively contains other backups.
 */
export function bookBackupsDir(root: string, bookId: string): string {
  return join(root, ".inoks-story-webnovel", "backups", bookId);
}
...
export async function restoreBookBackup(
  ...
  if (!backupInfo?.isDirectory()) {
    throw new Error(
      ...
      + `List available backups with: inoks-story book backup ${bookId} --list`,
    );
  }
