...
// Windows + OneDrive/Defender cold starts can spend more than 10s loading the
// complete Core command graph. Keep the child-process deadline above the
// command's own network timeouts so integration tests observe its real result.
...
const failingLlmEnv = {
  ...
  INOKS_STORY_LLM_BASE_URL: "http://127.0.0.1:9/v1",
  INOKS_STORY_LLM_MODEL: "test-model",
  INOKS_STORY_LLM_API_KEY: "test-key",
};

describe("CLI integration", () => {
  beforeAll(async () => {
    projectDir = await mkdtemp(join(tmpdir(), "inoks-story-cli-test-"));
  });
  ...
  describe("inoks-story --help", () => {
    it("prints help with command list", () => {
      const output = run(["--help"]);
      expect(output).toContain("inoks-story");
  ...
  describe("inoks-story init", () => {
    it("initializes project in current directory", () => {
      const output = run(["init"]);
      expect(output).toContain("Project initialized");
    });

    it("creates inoks-story-webnovel.json with correct structure", async () => {
      const raw = await readFile(join(projectDir, "inoks-story-webnovel.json"), "utf-8");
    ...
    it("creates .env file", async () => {
      ...
      expect(envContent).toContain("INOKS_STORY_LLM_API_KEY");
    });
  ...
  describe("inoks-story init <name>", () => {
    it("creates project in subdirectory", () => {
      const output = run(["init", "subproject"]);
      expect(output).toContain("Project initialized");
    });

    it("creates inoks-story-webnovel.json in subdirectory", async () => {
      const raw = await readFile(join(projectDir, "subproject", "inoks-story-webnovel.json"), "utf-8");
      const config = JSON.parse(raw);
      expect(config.name).toBe("subproject");
    });

    it("supports absolute project paths instead of nesting them under cwd", async () => {
      const absoluteDir = await mkdtemp(join(tmpdir(), "inoks-story-cli-abs-init-"));

      try {
        const output = run(["init", absoluteDir]);
        expect(output).toContain(`Project initialized at ${absoluteDir}`);

        const raw = await readFile(join(absoluteDir, "inoks-story-webnovel.json"), "utf-8");
        const config = JSON.parse(raw);
    ...
    it("prints English next steps when initialized with --lang en", async () => {
      const englishDir = await mkdtemp(join(tmpdir(), "inoks-story-cli-en-init-"));

      try {
        const output = run(["init", englishDir, "--lang", "en"]);
        expect(output).toContain("Project initialized");
        expect(output).toContain("inoks-story book create --title 'My Novel'");
  ...
  describe("inoks-story config set", () => {
  ...
  describe("inoks-story config show", () => {
  ...
  describe("inoks-story interact", () => {
    it("returns the agent-session JSON contract for natural-language interactions", async () => {
      const initialized = await stat(join(projectDir, "inoks-story-webnovel.json")).then(() => true).catch(() => false);
    ...
    it("binds the requested book when interact is called with --book", async () => {
      ...
      try {
        ...
        await rm(join(projectDir, ".inoks-story-webnovel-session.json"), { force: true }).catch(() => {});
      }
    }, CLI_PROCESS_TIMEOUT_MS);
  });

  describe("inoks-story config set-model", () => {
  ...
  describe("inoks-story book list", () => {
  ...
  describe("inoks-story book create", () => {
    it("removes stale incomplete book directories before retrying create", async () => {
      try {
        await stat(join(projectDir, "inoks-story-webnovel.json"));
      } catch {
        run(["init"]);
      }
    ...
    it("treats localhost OpenAI-compatible endpoints as API-key optional", async () => {
      ...
      try {
        ...
        await writeFile(envPath, [
          "INOKS_STORY_LLM_PROVIDER=openai",
          "INOKS_STORY_LLM_BASE_URL=http://127.0.0.1:11434/v1",
          "INOKS_STORY_LLM_MODEL=gpt-oss:20b",
          "",
        ].join("\n"), "utf-8");

        const { stdout } = runStderr(["doctor"], {
          env: { INOKS_STORY_LLM_API_KEY: "" },
        });
  ...
  describe("inoks-story analytics", () => {
  ...
  describe("inoks-story plan/compose", () => {
    beforeAll(async () => {
      const configPath = join(projectDir, "inoks-story-webnovel.json");
      const initialized = await stat(configPath).then(() => true).catch(() => false);
      if (!initialized) run(["init"]);
