...
describe("short command", () => {
  ...
  it("requires an explicit cover API key", () => {
    const oldValue = process.env.INOKS_STORY_TEST_MISSING_COVER_KEY;
    delete process.env.INOKS_STORY_TEST_MISSING_COVER_KEY;
    try {
      expect(() => resolveCoverApiKey("INOKS_STORY_TEST_MISSING_COVER_KEY")).toThrow(/API key/i);
    } finally {
      if (oldValue === undefined) delete process.env.INOKS_STORY_TEST_MISSING_COVER_KEY;
      else process.env.INOKS_STORY_TEST_MISSING_COVER_KEY = oldValue;
    }
  });
});
