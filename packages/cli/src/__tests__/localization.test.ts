...
describe("CLI localization", () => {
  it("formats book-create summaries in both languages", () => {
    ...
    expect(formatBookCreateNextStep("zh", "shan-he")).toBe("下一步：inoks-story write next shan-he");
    ...
    expect(formatBookCreateNextStep("en", "harbor")).toBe("Next: inoks-story write next harbor");
  });
  ...
  it("formats import summaries with language-specific units and action hints", () => {
    ...
    expect(formatImportChaptersComplete("zh", {
      ...
      "  下一章编号：13",
      "",
      '运行 "inoks-story write next shan-he" 继续写作。',
    ]);
    ...
    expect(formatImportChaptersComplete("en", {
      ...
      "",
      'Run "inoks-story write next harbor" to continue writing.',
    ]);
  });
...
describe("resolveCliLanguage environment fallback", () => {
  it("prefers the explicit language over any environment variable", () => {
    expect(resolveCliLanguage("en", { INOKS_STORY_LOCALE: "zh_CN" })).toBe("en");
    expect(resolveCliLanguage("zh", { INOKS_STORY_LOCALE: "en", LANG: "en_US.UTF-8" })).toBe("zh");
  });

  it("reads INOKS_STORY_LOCALE before the system locale variables", () => {
    expect(resolveCliLanguage(undefined, { INOKS_STORY_LOCALE: "en", LANG: "zh_CN.UTF-8" })).toBe("en");
    expect(resolveCliLanguage(undefined, { INOKS_STORY_LOCALE: "zh-CN", LC_ALL: "en_US.UTF-8" })).toBe("zh");
  });
...
describe("doctor hint localization", () => {
  it("keeps the original Chinese hints for zh", () => {
    ...
    expect(formatDoctorHintBaseUrl("zh")).toContain("INOKS_STORY_LLM_BASE_URL");
    expect(formatDoctorHintStreamRequirement("zh")).toContain("stream");
    expect(formatDoctorHintModelName("zh")).toContain("INOKS_STORY_LLM_MODEL");
    expect(formatDoctorHintInvalidApiKey("zh")).toContain("INOKS_STORY_LLM_API_KEY");
  ...
  it("emits pure English hints for en", () => {
    ...
    expect(formatDoctorHintBaseUrl("en")).toContain("INOKS_STORY_LLM_BASE_URL");
    expect(formatDoctorHintModelName("en")).toContain("INOKS_STORY_LLM_MODEL");
    expect(formatDoctorHintInvalidApiKey("en")).toContain("INOKS_STORY_LLM_API_KEY");
...
describe("fanfic error localization", () => {
  it("builds bilingual error messages", () => {
    ...
    expect(missingCanon).toContain("inoks-story fanfic init");
