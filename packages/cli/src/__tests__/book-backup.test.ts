...
async function setupBook(bookId: string): Promise<string> {
  projectRoot = await mkdtemp(join(tmpdir(), "inoks-story-book-backup-"));
...
describe("book backup module", () => {
  it("snapshots the whole book directory into .inoks-story-webnovel/backups/<bookId>/<stamp>/", async () => {
    ...
    const backupDir = join(projectRoot, ".inoks-story-webnovel", "backups", "backbook", "20260715-081233");
  ...
  it("restores a backup and auto-backs-up the current state first", async () => {
    ...
    const preRestoreDir = join(projectRoot, ".inoks-story-webnovel", "backups", "restorebook", "20260715-090000-pre-restore");
...
describe("inoks-story book backup / restore commands", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
