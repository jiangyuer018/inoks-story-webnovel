...
describe("project bootstrap", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), "inoks-story-bootstrap-"));
  ...
  it("creates a minimal Studio-first project when none exists", async () => {
    ...
    const config = JSON.parse(await readFile(join(tempDir, "inoks-story-webnovel.json"), "utf-8"));
    expect(config.name).toMatch(/^inoks-story-bootstrap-/);
  ...
  it("returns false when the directory is already an Inoks Story Webnovel project", async () => {
    await writeFile(join(tempDir, "inoks-story-webnovel.json"), "{}\n", "utf-8");
    const { ensureProjectDirectoryInitialized } = await import("../project-bootstrap.js");
