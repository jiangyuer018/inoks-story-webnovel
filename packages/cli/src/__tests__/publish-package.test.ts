...
const CLI_PACK_TEST_TIMEOUT_MS = 60_000;
...
describe.sequential("publish packaging", () => {
  it("rewrites workspace package versions for canary publishing", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "inoks-story-version-script-"));
    ...
    try {
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "inoks-story", version: "0.4.6" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCoreDir, "package.json"),
        `${JSON.stringify({ name: "@inoks-story-webnovel/core", version: "0.4.6" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
          {
            name: "@inoks-story-webnovel/cli",
            version: "0.4.6",
            dependencies: {
              "@inoks-story-webnovel/core": "workspace:*",
              commander: "^13.0.0",
            },
          },
          null,
      ...
      expect(cliPackageJson.dependencies["@inoks-story-webnovel/core"]).toBe("0.4.8-canary.7");
    } finally {
      await rm(tempRoot, { recursive: true, force: true });
    }
  });

  it("keeps source publish dependencies workspace-linked before prepack", async () => {
    const cliPackageJson = await sourceCliPackageJsonPromise;
    const studioPackageJson = await sourceStudioPackageJsonPromise;

    expect(cliPackageJson.dependencies["@inoks-story-webnovel/core"]).toBe("workspace:*");
    expect(cliPackageJson.dependencies["@inoks-story-webnovel/studio"]).toBe("workspace:*");
    expect(studioPackageJson.dependencies["@inoks-story-webnovel/core"]).toBe("workspace:*");
  });
  ...
  it("rejects workspace protocol in source publish manifests", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "inoks-story-publish-verify-pass-"));
    const tempPackagesDir = join(tempRoot, "packages");
    const tempCoreDir = join(tempPackagesDir, "core");
    const tempCliDir = join(tempPackagesDir, "cli");
    ...
    try {
      ...
      await writeFile(
        join(tempRoot, "package.json"),
        `${JSON.stringify({ name: "inoks-story", version: "0.5.1" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCoreDir, "package.json"),
        `${JSON.stringify({ name: "@inoks-story-webnovel/core", version: "0.5.1" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
  ...
  it("rejects workspace protocol manifests that normalize to the wrong internal version", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "inoks-story-publish-verify-fail-"));
    const tempPackagesDir = join(tempRoot, "packages");
    const tempCoreDir = join(tempPackagesDir, "core");
    const tempCliDir = join(tempPackagesDir, "cli");
        ...
        `${JSON.stringify(
          {
            ...
            version: "0.5.1",
            dependencies: {
              "@inoks-story-webnovel/core": "workspace:0.5.0",
            },
          },
          null,
          2,
  ...
  it("replaces workspace dependencies before npm pack", { timeout: CLI_PACK_TEST_TIMEOUT_MS }, async () => {
    const packDir = await mkdtemp(join(tmpdir(), "inoks-story-cli-pack-"));
    ...
    try {
      ...
      expect(packedPackageJson.dependencies["@inoks-story-webnovel/core"]).toBe(corePackageJson.version);
      expect(packedPackageJson.dependencies["@inoks-story-webnovel/studio"]).toBe(studioPackageJson.version);
    } finally {
  ...
  it("packs the studio runtime entry alongside the built frontend", { timeout: STUDIO_PACK_TEST_TIMEOUT_MS }, async () => {
    const packDir = await mkdtemp(join(tmpdir(), "inoks-story-studio-pack-"));
