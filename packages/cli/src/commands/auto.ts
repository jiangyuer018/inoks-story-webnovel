...
import { PipelineRunner, StateManager } from "@inoks-story-webnovel/core";
  ...
  .action(async (args: ReadonlyArray<string>, opts) => {
    ...
    try {
      ...
      if (args.length === 1) {
        ...
        throw new Error("Usage: inoks-story auto [book-id] <target-chapter>");
      }
      if (targetChapter < 1) {
      ...
      // `inoks-story auto` is unattended batch writing, so the audit→revise loop must
      ...
      for (let chapter = startChapter; chapter <= targetChapter; chapter++) {
        ...
        if (result.status === "state-degraded") {
          throw new Error(
            `Chapter ${result.chapterNumber} finished in state-degraded status, stopping auto-write. Run "inoks-story write repair-state ${bookId} ${result.chapterNumber}" first, then re-run inoks-story auto.`,
          );
        }
      }
