...
forecastCommand
  ...
  .action(async (bookIdArg: string | undefined, opts: ForecastCreateOptions) => {
    try {
      ...
      log(`Select a branch with: inoks-story forecast select ${bookId} ${result.forecast.forecastId} <branch-id>`);
    } catch (e) {
...
export function parseForecastShowArgs(args: ReadonlyArray<string>): {
  ...
  throw new Error("Usage: inoks-story forecast show [book-id] <forecast-id>");
}
...
export function parseForecastSelectArgs(args: ReadonlyArray<string>): {
  ...
  throw new Error("Usage: inoks-story forecast select [book-id] <forecast-id> <branch-id>");
}
