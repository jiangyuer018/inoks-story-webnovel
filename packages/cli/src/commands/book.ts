...
import { deriveBookIdFromTitle, normalizePlatformOrOther, PipelineRunner, StateManager, type BookConfig } from "@inoks-story-webnovel/core";
import {
    ...
    try {
      ...
      if (opts.json) {
        log(JSON.stringify({
          ...
          nextStep: `inoks-story write next ${bookId}`,
        }, null, 2));
      } else {
    ...
    try {
      ...
      if (bookIds.length === 0) {
        if (opts.json) {
          ...
          log("No books found. Create one with: inoks-story book create --title '...'");
        }
        return;
      }
...
bookCommand
  .command("backup")
  .description("Snapshot the whole book directory into .inoks-story-webnovel/backups/<book-id>/ (or list backups with --list)")
...
bookCommand
  ...
  .argument("<book-id>", "Book ID")
  .argument("<backup-id>", "Backup ID, see `inoks-story book backup <book-id> --list`")
  .option("--json", "Output JSON")
