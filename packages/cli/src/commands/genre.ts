...
import { listAvailableGenres, readGenreProfile, getBuiltinGenresDir } from "@inoks-story-webnovel/core";
...
genreCommand
  ...
  .option("--lang <language>", "Template language: zh or en (defaults to INOKS_STORY_LOCALE/LANG, then zh)")
  ...
  .action(async (id: string) => {
    try {
      ...
      try {
        ...
        logError(`Built-in genre "${id}" not found. Use 'inoks-story genre list' to see available genres.`);
        process.exit(1);
        return;
      }
