...
export { extractResponsesImageBase64, resolveCoverApiKey } from "@inoks-story-webnovel/core";
...
shortCommand
  ...
  .option("--cover-api-key-env <name>", "Env var containing cover API key", "INOKS_STORY_COVER_API_KEY")
  ...
  .action(async (opts: ShortRunOptions) => {
    try {
      ...
      const result = await runShortFictionProduction({
        ...
        proseQuality: (await loadConfig({ projectRoot: root })).writing.proseQuality,
      });
...
async function createShortRuntime(
  ...
  try {
    ...
    if (!String(e).includes("inoks-story-webnovel.json not found")) throw e;
...
function buildEnvLLMConfig(options: {
  ...
  const baseUrl = options.llmBaseUrl ?? process.env.INOKS_STORY_LLM_BASE_URL;
  const model = options.model ?? process.env.INOKS_STORY_LLM_MODEL;
  if (!baseUrl) throw new Error("LLM base URL is required. Set INOKS_STORY_LLM_BASE_URL or pass --llm-base-url.");
  if (!model) throw new Error("LLM model is required. Set INOKS_STORY_LLM_MODEL or pass --model.");
  return {
    provider: "openai",
    service: process.env.INOKS_STORY_LLM_SERVICE ?? "custom",
    configSource: "env",
    baseUrl,
    apiKey: process.env.INOKS_STORY_LLM_API_KEY ?? "",
    model,
    temperature: parseEnvNumber(process.env.INOKS_STORY_LLM_TEMPERATURE, 0.1),
    thinkingBudget: parseEnvInteger(process.env.INOKS_STORY_LLM_THINKING_BUDGET, 0),
    apiFormat: process.env.INOKS_STORY_LLM_API_FORMAT === "responses" ? "responses" : "chat",
    stream: process.env.INOKS_STORY_LLM_STREAM === "false" ? false : true,
  };
}
