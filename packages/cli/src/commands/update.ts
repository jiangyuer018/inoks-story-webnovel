...
export const updateCommand = new Command("update")
  .description("Update Inoks Story Webnovel to the latest version")
  .action(async () => {
    try {
      ...
      const remoteVersion = execSync("npm view @inoks-story-webnovel/cli version", {
        encoding: "utf-8",
      }).trim();
      ...
      execSync("npm install -g @inoks-story-webnovel/cli@latest", { stdio: "inherit" });
      log(`Updated to ${remoteVersion}.`);
    } catch (e) {
      logError(`Update failed: ${e}`);
      log("You can also update manually: npm install -g @inoks-story-webnovel/cli@latest");
      process.exit(1);
    }
  });
