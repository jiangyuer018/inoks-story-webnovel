...
import { listModelsForService } from "@inoks-story-webnovel/core";
...
configCommand
  .command("set-global")
  .description("Set global LLM config (~/.inoks-story-webnovel/.env), shared by all projects")
  ...
  .action(async (opts) => {
    try {
      ...
      const lines = [
        "# Inoks Story Webnovel Global LLM Configuration",
        `INOKS_STORY_LLM_PROVIDER=${opts.provider}`,
        `INOKS_STORY_LLM_BASE_URL=${opts.baseUrl}`,
        `INOKS_STORY_LLM_API_KEY=${opts.apiKey}`,
        `INOKS_STORY_LLM_MODEL=${opts.model}`,
      ];
      if (opts.temperature) lines.push(`INOKS_STORY_LLM_TEMPERATURE=${opts.temperature}`);
      if (opts.thinkingBudget) lines.push(`INOKS_STORY_LLM_THINKING_BUDGET=${opts.thinkingBudget}`);
      if (opts.apiFormat) lines.push(`INOKS_STORY_LLM_API_FORMAT=${opts.apiFormat}`);
      if (opts.lang) lines.push(`INOKS_STORY_DEFAULT_LANGUAGE=${opts.lang}`);
...
configCommand
  .command("show-global")
  .description("Show global LLM config (~/.inoks-story-webnovel/.env)")
  .action(async () => {
    try {
      const content = await readFile(GLOBAL_ENV_PATH, "utf-8");
      const masked = content.replace(
        /(INOKS_STORY_LLM_API_KEY=)(.{8})(.*)(.{4})/,
        "$1$2...$4",
      );
      log(masked);
    } catch {
      log("No global config found. Run 'inoks-story config set-global' to create one.");
    }
  });
...
configCommand
  ...
  .option("--api-key <key>", "API Key (also reads from INOKS_STORY_LLM_API_KEY env)")
  ...
  .action(async (service: string, opts: { apiKey?: string; baseUrl?: string; json?: boolean }) => {
    const apiKey = opts.apiKey ?? process.env.INOKS_STORY_LLM_API_KEY;
