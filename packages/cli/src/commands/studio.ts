...
export async function resolveStudioLaunch(root: string): Promise<StudioLaunchSpec | null> {
  ...
  const builtEntry = await firstAccessiblePath([
    join(root, "node_modules", "@inoks-story-webnovel", "studio", "dist", "api", "index.js"),
    join(root, "node_modules", "@inoks-story-webnovel", "studio", "server.cjs"),
    join(cliPackageRoot, "node_modules", "@inoks-story-webnovel", "studio", "dist", "api", "index.js"),
    join(cliPackageRoot, "node_modules", "@inoks-story-webnovel", "studio", "server.cjs"),
...
export async function launchStudioWorkbench(root: string, port: string): Promise<void> {
  ...
  if (!launch) {
    logError(
      "Inoks Story Webnovel Studio not found. If you cloned the repo, run:\n" +
      "  cd packages/studio && pnpm install && pnpm build\n" +
      "Then run 'inoks-story studio' from the project root.",
    );
    process.exit(1);
  }

  log(`Starting Inoks Story Webnovel Studio on ${url}`);

  const child = spawn(launch.command, launch.args, {
    cwd: root,
    stdio: "inherit",
    env: { ...process.env, INOKS_STORY_STUDIO_PORT: port },
  });
...
export async function launchStudioEntry(
  ...
  if (prepared.initialized) {
    log(`No inoks-story-webnovel.json found in ${root}. Initialized a minimal Inoks Story Webnovel project for Studio.`);
  }
