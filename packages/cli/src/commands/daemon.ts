...
import { Scheduler } from "@inoks-story-webnovel/core";
import { loadConfig, findProjectRoot, buildPipelineConfig, log, logError } from "../utils.js";
import { createWriteStream, type WriteStream } from "node:fs";
import { writeFile, readFile, unlink } from "node:fs/promises";
import { join } from "node:path";

const PID_FILE = "inoks-story.pid";

export const upCommand = new Command("up")
  .description("Start the Inoks Story Webnovel daemon (autonomous mode)")
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      try {
        ...
        logError(`Daemon already running (PID: ${existingPid.trim()}). Run 'inoks-story down' first.`);
        process.exit(1);
      } catch {
        // No PID file, good
      }

      log("Starting Inoks Story Webnovel daemon...");
...
export const downCommand = new Command("down")
  .description("Stop the Inoks Story Webnovel daemon")
  .action(async () => {
    const root = findProjectRoot();
