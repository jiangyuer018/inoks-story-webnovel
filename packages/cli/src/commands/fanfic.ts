...
import { deriveBookIdFromTitle, normalizePlatformOrOther, PipelineRunner, type BookConfig, type FanficMode } from "@inoks-story-webnovel/core";
  ...
  .action(async (opts) => {
    try {
      ...
      if (opts.json) {
        ...
        log("");
        log(`Next: inoks-story write next ${bookId}`);
      }
    } catch (e) {
      if (opts.json) {
