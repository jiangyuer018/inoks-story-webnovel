  ...
  .action(async (name: string | undefined, opts: { lang?: string }) => {
    ...
    try {
      ...
      const exampleCreateLines = isEnglish
        ? ["  inoks-story book create --title 'My Novel' --genre progression --platform royalroad --lang en"]
        : [
      ...
      if (globalConfigured) {
        ...
        log("  inoks-story config set-global --provider openai --base-url <your-api-url> --api-key <your-key> --model <your-model>");
        log("  # Option 2: Edit .env for this project only");
        log("");
        for (const line of exampleCreateLines) log(line);
      }
      log("  inoks-story write next <book-id>");
    } catch (e) {
