...
import { fetchWithProxy } from "@inoks-story-webnovel/core";
import {
...
export const doctorCommand = new Command("doctor")
  ...
  .action(async (opts: { repairNodeRuntime?: boolean }) => {
    ...
    // (INOKS_STORY_LOCALE -> LC_ALL/LC_MESSAGES/LANG, default zh).
    ...
    // 2. Check inoks-story-webnovel.json exists
    try {
      await readFile(join(root, "inoks-story-webnovel.json"), "utf-8");
      checks.push({ name: "inoks-story-webnovel.json", ok: true, detail: "Found" });
    } catch {
      checks.push({ name: "inoks-story-webnovel.json", ok: false, detail: "Not found. Run 'inoks-story init'" });
    }
    ...
    // 4. Check global config
    {
      ...
      try {
        ...
        hasGlobal = globalContent.includes("INOKS_STORY_LLM_API_KEY=") && !globalContent.includes("your-api-key-here");
      } catch { /* no global config */ }
      checks.push({
        name: "Global Config",
        ok: hasGlobal,
        detail: hasGlobal ? `Found (${GLOBAL_ENV_PATH})` : "Not set. Run 'inoks-story config set-global'",
      });
    }
    ...
    // 5. Check effective LLM config (Studio project base + env/CLI overlay, or legacy env)
    {
      ...
      const { isApiKeyOptionalForEndpoint } = await import("@inoks-story-webnovel/core");
    ...
    try {
      const { StateManager } = await import("@inoks-story-webnovel/core");
      ...
      if (hasStructuredState) {
        ...
        if (legacyCount > 0) {
          checks.push({
            ...
            ok: false,
            detail: `${legacyCount} book(s) using legacy format (pre-v0.6). Run 'inoks-story write next' on each to auto-migrate, or re-init with 'inoks-story init'.`,
          });
    ...
    try {
      const { createLLMClient, chatCompletion, LLMConfigSchema, isApiKeyOptionalForEndpoint, resolveServiceModelsBaseUrl } = await import("@inoks-story-webnovel/core");
      ...
      try {
        ...
        const apiKeyOptional = isApiKeyOptionalForEndpoint({
          provider: env.INOKS_STORY_LLM_PROVIDER,
          baseUrl: env.INOKS_STORY_LLM_BASE_URL,
        });
        if ((env.INOKS_STORY_LLM_API_KEY || apiKeyOptional) && env.INOKS_STORY_LLM_BASE_URL && env.INOKS_STORY_LLM_MODEL) {
          llmConfig = LLMConfigSchema.parse({
            provider: env.INOKS_STORY_LLM_PROVIDER ?? "custom",
            baseUrl: env.INOKS_STORY_LLM_BASE_URL,
            apiKey: env.INOKS_STORY_LLM_API_KEY ?? "",
            model: env.INOKS_STORY_LLM_MODEL,
          });
        }
      }
      ...
      if (!llmConfig) {
        ...
        checks.push({
          name: "  Hint",
          ok: false,
          detail: "Run `inoks-story setup`, `inoks-story config set-global`, or add LLM settings to the project .env file.",
        });
      } else {
        checks.push({
          name: "LLM Config",
