...
const ZH_CN: TuiCopy = {
  ...
  composer: {
    placeholder: "告诉 Inoks Story Webnovel 要写什么、修改什么，或解释什么…",
    emptyConversation: "先告诉 Inoks Story Webnovel 你要做什么。",
  ...
  notes: {
    ...
    config: "当前 Ink 仪表盘里还不支持交互式 /config。请使用 inoks-story config set-global。",
  ...
  roles: {
    user: "你",
    assistant: "Inoks Story Webnovel",
    system: "系统",
  },
  activity: {
    thinking: "思考中",
...
const EN: TuiCopy = {
  ...
  composer: {
    placeholder: "Ask Inoks Story Webnovel to write, revise, or explain…",
    emptyConversation: "Start by asking Inoks Story Webnovel what to do.",
  ...
  notes: {
    ...
    config: "Interactive /config is not available inside the Ink dashboard yet. Use inoks-story config set-global.",
...
export function resolveTuiLocale(
  ...
  const requested = normalizeLocale(env.INOKS_STORY_TUI_LOCALE ?? env.INOKS_STORY_LOCALE);
  if (requested) {
    return requested;
  }
