...
export function buildAutoInitMessages(projectName: string, locale: TuiLocale): {
  ...
  if (locale === "en") {
    return {
      ...
      envTemplateHeader: "# LLM Configuration — run inoks-story tui to configure interactively",
    };
  }
  ...
  return {
    ...
    initialized: "项目已初始化",
    envTemplateHeader: "# LLM 配置 —— 运行 inoks-story tui 进行交互式配置",
  };
}

export async function ensureProject(cwd: string): Promise<SetupResult> {
  const configPath = join(cwd, "inoks-story-webnovel.json");
...
export async function interactiveLlmSetup(
  ...
  try {
    ...
    const envContent = [
      `INOKS_STORY_LLM_PROVIDER=${finalProvider}`,
      ...(finalService ? [`INOKS_STORY_LLM_SERVICE=${finalService}`] : []),
      `INOKS_STORY_LLM_BASE_URL=${effectiveBaseUrl}`,
      `INOKS_STORY_LLM_API_KEY=${apiKey.trim()}`,
      `INOKS_STORY_LLM_MODEL=${model.trim()}`,
    ].join("\n");
...
async function autoInit(cwd: string): Promise<void> {
  ...
  const config = {
    ...
    language: "zh",
    llm: {
      provider: process.env.INOKS_STORY_LLM_PROVIDER ?? "openai",
      baseUrl: process.env.INOKS_STORY_LLM_BASE_URL ?? "",
      model: process.env.INOKS_STORY_LLM_MODEL ?? "",
    },
    notify: [],
    daemon: {
      schedule: {
  ...
  await writeFile(
    join(cwd, "inoks-story-webnovel.json"),
...
async function checkEnvForKey(envPath: string): Promise<boolean> {
  try {
    ...
    const match = content.match(/INOKS_STORY_LLM_API_KEY=(.+)/);
...
export async function detectProjectLanguage(projectRoot: string): Promise<string | undefined> {
  try {
    const raw = await readFile(join(projectRoot, "inoks-story-webnovel.json"), "utf-8");
...
async function parseEnvModel(envPath: string): Promise<ModelInfo | undefined> {
  try {
    ...
    const key = get("INOKS_STORY_LLM_API_KEY");
    if (!key || key.includes("your-api-key")) return undefined;
    return {
      provider: get("INOKS_STORY_LLM_PROVIDER") || "openai",
      model: get("INOKS_STORY_LLM_MODEL") || "unknown",
      baseUrl: get("INOKS_STORY_LLM_BASE_URL") || "",
    };
  } catch {
    return undefined;
  }
