...
describe("Studio skill endpoints", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-studio-skills-"));
  });
  ...
  it("lists built-in skills and project-local skills", async () => {
    await mkdir(join(root, ".inoks-story-webnovel", "skills", "detective-play"), { recursive: true });
    await writeFile(
      join(root, ".inoks-story-webnovel", "skills", "detective-play", "SKILL.md"),
      [
        "---",
        "id: detective-play",
        "name: Detective Play",
  ...
  it("creates, updates, and deletes project skills as SKILL.md files", async () => {
    ...
    expect(await readFile(join(root, ".inoks-story-webnovel", "skills", "romance-play", "SKILL.md"), "utf-8"))
