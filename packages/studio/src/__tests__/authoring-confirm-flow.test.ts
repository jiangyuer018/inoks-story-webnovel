...
import { createAndPersistBookSession, loadStoryGraph } from "@inoks-story-webnovel/core";

const INOKS_STORY_CONFIG = JSON.stringify({
  name: "test-project",
  version: "0.1.0",
  language: "zh",
...
describe("interactive-film-authoring confirm flow (stubbed LLM)", () => {
  let root: string;
  const prev = process.env.INOKS_STORY_AGENT_LLM_STUB;
  beforeAll(() => { process.env.INOKS_STORY_AGENT_LLM_STUB = "1"; });
  afterAll(() => {
    if (prev === undefined) delete process.env.INOKS_STORY_AGENT_LLM_STUB;
    else process.env.INOKS_STORY_AGENT_LLM_STUB = prev;
  });
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "if-confirm-"));
    await writeFile(join(root, "inoks-story-webnovel.json"), INOKS_STORY_CONFIG, "utf-8");
