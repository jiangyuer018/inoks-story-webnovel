...
const KNOWN_RUNTIME_REPLACEMENTS: ReadonlyArray<{
  ...
  {
    pattern: /INOKS_STORY_LLM_API_KEY not set\. Run 'inoks-story config set-global' or add it to project \.env file\./g,
    replacement: "INOKS_STORY_LLM_API_KEY 未设置。请运行 `inoks-story config set-global`，或在项目 .env 文件中添加它。",
  },
];
