...
import { InoksStoryLogo } from "./InoksStoryLogo";
...
interface Nav {
  ...
  toStoryWorkbench: (id: string) => void;
  toBookCreate: () => void;
  toServices: () => void;
...
export function Sidebar({ nav, activePage, sse, t }: {
  ...
  useEffect(() => {
    const match = activePage.match(/^(?:book|workbench):(.+)$/);
    const bookId = match?.[1];
    if (!bookId) return;
    setExpandedBooks((previous) => {
      if (previous.has(bookId)) return previous;
  ...
  }, [activePage, loadSessionList, sessionIdsByBook]);
  ...
  return (
    <aside className="w-[260px] shrink-0 border-r border-border bg-background/80 backdrop-blur-md flex flex-col h-full overflow-hidden select-none">
      ...
      <div className="px-6 py-8">
        ...
        >
          <InoksStoryLogo className="w-11 h-11 shrink-0 group-hover:scale-105 transition-transform" />
          <div className="flex flex-col">
            <span className="font-serif text-[27px] leading-none italic font-medium">Inoks Story Webnovel</span>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2 space-y-6">
        {/* Inoks Story Webnovel Create Section — always visible, two columns. */}
        <div>
        ...
        <div>
          ...
          <div className="space-y-0.5 pt-1">
            {books.map((book) => {
              ...
              const isActiveWorkbench = activePage === `workbench:${book.id}`;
                  ...
                  <div className="group/book flex items-center">
                    ...
                    <button
                      ...
                      className={`flex min-w-0 flex-1 items-center gap-1.5 py-1.5 pr-2 rounded-md text-[15px] leading-6 transition-colors ${
                        isActiveBook || isActiveWorkbench ? "text-foreground font-medium" : "text-muted-foreground hover:text-foreground hover:bg-secondary/30"
                      }`}
                    >
                  ...
                  <Collapse open={isExpanded}>
                    <div className="mt-0.5">
                      <button
                        ...
                        className={`w-full flex items-center gap-2 pl-9 pr-2 py-1.5 rounded-md text-left text-[14px] leading-5 transition-colors ${
                          isActiveWorkbench
                      ...
                      >
                        ...
                        <span>{tr("故事控制", "Story control")}</span>
                      </button>
