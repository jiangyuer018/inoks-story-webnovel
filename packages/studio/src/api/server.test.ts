...
vi.mock("@inoks-story-webnovel/core", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@inoks-story-webnovel/core")>();
  ...
  return {
    ...
    GLOBAL_ENV_PATH: join(tmpdir(), "inoks-story-global.env"),
    ...
    ChapterApprovalStore: actual.ChapterApprovalStore,
    latestProjectionFailures: actual.latestProjectionFailures,
    replayStorySystem: actual.replayStorySystem,
    repairStorySystem: actual.repairStorySystem,
    runStorySystemPreflight: actual.runStorySystemPreflight,
...
describe("createStudioServer daemon lifecycle", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "inoks-story-studio-server-"));
    await writeFile(join(root, "inoks-story-webnovel.json"), JSON.stringify(projectConfig, null, 2), "utf-8");
    ...
    applyManualChapterAmendmentMock.mockImplementation(async (
      _bookId: string,
      _chapterNumber: number,
      content: string,
    ) => {
      await writeFile(join(root, "books", "demo-book", "chapters", "0003_Demo.md"), content, "utf-8");
    });
    ...
    loadProjectConfigMock.mockImplementation(async () => {
      const raw = JSON.parse(await readFile(join(root, "inoks-story-webnovel.json"), "utf-8")) as Record<string, unknown>;
      return {
        ...cloneProjectConfig(),
        ...raw,
        llm: {
  ...
  afterEach(async () => {
    ...
    await rm(join(tmpdir(), "inoks-story-global.env"), { force: true });
  });

  it("uses the real core bookId validator in the Studio safety mock", async () => {
    const { isSafeBookId } = await import("@inoks-story-webnovel/core");
  ...
  it("returns a structured config error when inoks-story-webnovel.json is corrupt", async () => {
    await writeFile(join(root, "inoks-story-webnovel.json"), "{ this is not valid json", "utf-8");
    ...
    expect(body.error.message).toContain("inoks-story-webnovel.json");
  });
  ...
  it("returns all bank services with group fields and custom services", async () => {
    await writeFile(join(root, "inoks-story-webnovel.json"), JSON.stringify({
      ...projectConfig,
      llm: {
        services: [
  ...
  it("reports config source and detected env overrides for Studio switching", async () => {
    await writeFile(join(root, ".env"), [
      "INOKS_STORY_LLM_PROVIDER=openai",
      "INOKS_STORY_LLM_BASE_URL=https://project.example.com/v1",
      "INOKS_STORY_LLM_MODEL=gpt-5.4",
      "INOKS_STORY_LLM_API_KEY=sk-project",
    ].join("\n"), "utf-8");
    await writeFile(join(tmpdir(), "inoks-story-global.env"), [
      "INOKS_STORY_LLM_PROVIDER=openai",
      "INOKS_STORY_LLM_BASE_URL=https://global.example.com/v1",
      "INOKS_STORY_LLM_MODEL=gpt-4o",
      "INOKS_STORY_LLM_API_KEY=sk-global",
    ].join("\n"), "utf-8");
    await writeFile(join(root, "inoks-story-webnovel.json"), JSON.stringify({
      ...projectConfig,
      llm: {
  ...
  it("imports detected env config into Studio services without exposing the key", async () => {
    await writeFile(join(tmpdir(), "inoks-story-global.env"), [
      "INOKS_STORY_LLM_PROVIDER=openai",
      "INOKS_STORY_LLM_BASE_URL=https://api.kkaiapi.com/v1",
      "INOKS_STORY_LLM_MODEL=deepseek-v4-flash",
      "INOKS_STORY_LLM_API_KEY=sk-global",
    ].join("\n"), "utf-8");
    loadSecretsMock.mockResolvedValue({ services: {} });
  ...
  it("does not probe stale global fallback models for custom services when /models is unavailable", async () => {
    ...
    await writeFile(join(root, ".env"), [
      "INOKS_STORY_LLM_MODEL=MiniMax-M2.7",
      "INOKS_STORY_LLM_BASE_URL=https://api.minimax.com/v1",
      "INOKS_STORY_LLM_API_KEY=sk-minimax",
    ].join("\n"), "utf-8");
  ...
  it("serves generated project cover images without exposing arbitrary files", async () => {
    ...
    const traversal = await app.request("http://localhost/api/v1/project/files/../inoks-story-webnovel.json");
  ...
  it("reads and writes generated text artifacts without exposing arbitrary files", async () => {
    ...
    const traversal = await app.request("http://localhost/api/v1/project/artifacts/interactive-films/%2e%2e/inoks-story-webnovel.json");
    expect([400, 404]).toContain(traversal.status);
  });
  ...
  it("reports async create failures through the create-status endpoint", async () => {
    processProjectInteractionRequestMock.mockRejectedValueOnce(new Error("INOKS_STORY_LLM_API_KEY not set"));
    ...
    await expect(status.json()).resolves.toMatchObject({
      status: "error",
      error: "INOKS_STORY_LLM_API_KEY not set",
    });
  });
  ...
  async function wireRealSessionTranscript() {
    const actual = await vi.importActual<typeof import("@inoks-story-webnovel/core")>("@inoks-story-webnovel/core");
  ...
  it("tags task pipeline log broadcasts with the execution id while chat round logs stay untagged", async () => {
    ...
    const core = await import("@inoks-story-webnovel/core");
    const { createStudioServer } = await import("./server.js");
    const app = createStudioServer(cloneProjectConfig() as never, root);
  ...
  it("returns BOOK_BUSY when direct write-next collides with an active write", async () => {
    const lockError = 'Book "demo-book" is locked by an active Inoks Story Webnovel write. Wait for it to finish or stop the running task, then retry.';
  ...
  it("passes configured long-form writing review retries into Studio write-next", async () => {
    await writeFile(
      join(root, "inoks-story-webnovel.json"),
      JSON.stringify({
        ...cloneProjectConfig(),
  ...
  it("handles explicit chat chapter edits outside the Inoks Story Webnovel writing agent", async () => {
    ...
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      response: expect.stringContaining("ChapterAmendment"),
      session: {
        sessionId: "agent-session-1",
        activeBookId: "demo-book",
      },
    ...
    expect(applyManualChapterAmendmentMock).toHaveBeenCalledWith(
      "demo-book",
      3,
      expect.stringContaining("Body updated"),
    );
    expect(saveChapterIndexMock).not.toHaveBeenCalled();
    expect(runAgentSessionMock).not.toHaveBeenCalled();
    expect(writeNextChapterMock).not.toHaveBeenCalled();
  ...
  it("handles explicit chat artifact edits only for content roots", async () => {
    ...
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      response: expect.stringContaining("已编辑 covers/demo/cover-prompt.md"),
    });
  ...
  it("handles explicit chat edits against role-card truth files", async () => {
    ...
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      response: expect.stringContaining("已编辑 books/demo-book/story/roles/主要角色/林月.md"),
    });
    await expect(readFile(rolePath, "utf-8")).resolves.toContain("查清账册里的失踪名单");
    expect(runAgentSessionMock).not.toHaveBeenCalled();
  });

  it("rejects chat edits to derived truth projections after the first accepted commit", async () => {
    const bookDir = join(root, "books", "demo-book");
    const statePath = join(bookDir, "story", "current_state.md");
    await mkdir(join(bookDir, "story"), { recursive: true });
    await mkdir(join(bookDir, ".inoks-story-webnovel", "story-system"), { recursive: true });
    await writeFile(statePath, "# Current State\n\n林月持有旧账册。\n", "utf-8");
    await writeFile(join(bookDir, ".inoks-story-webnovel", "story-system", "HEAD"), "commit-canonical\n", "utf-8");
    ...
    const response = await app.request("http://localhost/api/v1/agent", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        instruction: "把 books/demo-book/story/current_state.md 里的「持有旧账册」改成「丢失旧账册」",
        activeBookId: "demo-book",
        sessionId: "agent-session-1",
        sessionKind: "edit",
        requestedIntent: "edit_artifact",
    ...
    expect(response.status).toBe(409);
    await expect(response.json()).resolves.toMatchObject({
      error: expect.objectContaining({ code: "STORY_PROJECTION_READ_ONLY" }),
    });
    await expect(readFile(statePath, "utf-8")).resolves.toContain("持有旧账册");
    expect(runAgentSessionMock).not.toHaveBeenCalled();
    expect(applyManualChapterAmendmentMock).not.toHaveBeenCalled();
  });
  ...
  it("classifies Inoks Story Webnovel parser/tool errors as internal instead of blaming the selected provider", async () => {
    ...
    expect(json.error.message).toContain("Inoks Story Webnovel 内部流程错误");
  ...
  it("returns an active book write lock as BOOK_BUSY instead of a provider error", async () => {
    const lockError = 'Book "demo-book" is locked by an active Inoks Story Webnovel write (pid:123). Wait for it to finish or stop the running task, then retry. Stale locks are recovered automatically.';
    runAgentSessionMock.mockResolvedValueOnce({
      responseText: "",
  ...
  it("uploads a translation source, creates a translation project, lists it, and exports markdown", async () => {
    ...
    expect(uploaded.storedPath).toMatch(/^\.inoks-story-webnovel\/uploads\/translation\//);
