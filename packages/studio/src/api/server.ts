...
async function normalizeAgentAttachments(
  ...
  const uploadDir = join(root, ".inoks-story-webnovel", "uploads", safeUploadFileName(sessionId));
...
async function storeTranslationUpload(
  ...
  const uploadDir = join(root, ".inoks-story-webnovel", "uploads", "translation");
  await mkdir(uploadDir, { recursive: true });
...
function projectSkillsDir(root: string): string {
  return join(root, ".inoks-story-webnovel", "skills");
}
...
const CANONICAL_DERIVED_TRUTH_FILES = new Set([
  "current_state.md",
  "pending_hooks.md",
...
function resolveExternalChatEditPath(root: string, requestedPath: string): { path: string; rel: string } {
  ...
  if (rel.includes("/.inoks-story-webnovel/") || rel.endsWith("/.inoks-story-webnovel") || rel.includes("/secrets") || rel.endsWith(".env")) {
...
function parseDerivedTruthProjectionFromRelativePath(
  rel: string,
): { bookId: string; file: string } | null {
  const match = rel.match(/^books\/([^/]+)\/story\/([^/]+)$/);
  if (!match?.[1] || !match[2] || !CANONICAL_DERIVED_TRUTH_FILES.has(match[2])) return null;
  return { bookId: match[1], file: match[2] };
}

async function hasStorySystemHead(bookDir: string): Promise<boolean> {
  return readFile(join(bookDir, ".inoks-story-webnovel", "story-system", "HEAD"), "utf-8")
    .then((value) => Boolean(value.trim()), () => false);
}
...
async function tryHandleExternalChatEdit(params: {
  ...
  readonly applyChapterAmendment: (
    bookId: string,
  ...
  if (explicitPath) {
    ...
    const derivedTruthTarget = parseDerivedTruthProjectionFromRelativePath(target.rel);
    if (derivedTruthTarget && await hasStorySystemHead(
      join(params.root, "books", derivedTruthTarget.bookId),
    )) {
      throw new ApiError(
        409,
        "STORY_PROJECTION_READ_ONLY",
        `${derivedTruthTarget.file} is a derived ChapterCommit projection. Change canon through a chapter amendment and replay projections.`,
      );
    }
    ...
    if (chapterTarget) {
      await params.applyChapterAmendment(chapterTarget.bookId, chapterTarget.chapterNumber, updated);
    } else {
    ...
    return {
      ...
      responseText: chapterTarget
        ? `已通过质量门和连续性审查，将 ${target.rel} 记录为 ChapterAmendment。`
        : `已编辑 ${target.rel}。`,
    };
  }
  ...
  const updated = content.slice(0, first) + replacement.newText + content.slice(first + replacement.oldText.length);
  await params.applyChapterAmendment(params.activeBookId, chapterNumber, updated);

  return {
    activeBookId: params.activeBookId,
    responseText: `已通过质量门和连续性审查，将 ${params.activeBookId} 第 ${chapterNumber} 章修改记录为 ChapterAmendment。`,
  };
}
...
function classifyAgentFailure(message: string): AgentFailureKind {
  ...
  if (/BookWriteLockError|locked by an active Inoks Story Webnovel write|BOOK_BUSY/i.test(text)) {
    return "busy";
  }
  if (
...
function formatAgentFailure(
  ...
  if (kind === "internal") {
    return {
      ...
      message: pick(lang, `Inoks Story Webnovel 内部流程错误：${message}`, `Inoks Story Webnovel internal pipeline error: ${message}`),
      status: 500,
    };
  }
...
async function saveRawConfig(root: string, config: Record<string, unknown>): Promise<void> {
  await writeFile(join(root, "inoks-story-webnovel.json"), JSON.stringify(config, null, 2), "utf-8");
}
...
async function readEnvConfigValues(path: string): Promise<EnvConfigValues> {
  try {
    ...
    const provider = values.get("INOKS_STORY_LLM_PROVIDER") ?? null;
    const service = values.get("INOKS_STORY_LLM_SERVICE") ?? null;
    const baseUrl = values.get("INOKS_STORY_LLM_BASE_URL") ?? null;
    const model = values.get("INOKS_STORY_LLM_MODEL") ?? null;
    const apiKey = values.get("INOKS_STORY_LLM_API_KEY") ?? "";
...
export function createStudioServer(initialConfig: ProjectConfig, root: string, overrides: { readonly nodeImageGenerator?: NodeImageDeps } = {}) {
  ...
  app.onError((error, c) => {
    ...
    const message = error instanceof Error ? error.message : String(error);
    if (message.includes("LLM API key not set") || message.includes("INOKS_STORY_LLM_API_KEY not set")) {
  ...
  // Read the project language fresh from inoks-story-webnovel.json on every call, so a language
  // switch takes effect on the next request instead of being frozen at startup.
  // A missing/corrupt inoks-story-webnovel.json means "no project language configured" -> zh.
  ...
  app.get("/api/v1/genres", async (c) => {
    const { listAvailableGenres, readGenreProfile } = await import("@inoks-story-webnovel/core");
  ...
  app.get("/api/v1/books/:id/create-status", async (c) => {
    ...
    const { isBookFoundationComplete } = await import("@inoks-story-webnovel/core");
  ...
  app.put("/api/v1/books/:id/chapters/:num", async (c) => {
    ...
    try {
      ...
      const result = await pipeline.applyManualChapterAmendment(id, num, content);
      return c.json({ ok: true, chapterNumber: num, amendment: result });
    } catch (e) {
  ...
  app.get("/api/v1/books/:id/truth/:file{.+}", async (c) => {
    ...
    const { isNewLayoutBook, tryParseBookRulesFrontmatter } = await import("@inoks-story-webnovel/core");
  ...
  app.get("/api/v1/services", async (c) => {
    ...
    // Add custom services from inoks-story-webnovel.json
    try {
      const config = await loadRawConfig(root);
    ...
    if (!env || !env.values.apiKey) {
      return c.json({
        error: pick(
          ...
          "未检测到可导入的 LLM 环境变量配置，或缺少 INOKS_STORY_LLM_API_KEY。",
          "No importable LLM environment variable configuration was detected, or INOKS_STORY_LLM_API_KEY is missing.",
        ),
      }, 400);
    }
  ...
  app.get("/api/v1/cover/config", async (c) => {
    ...
    const envConfigured = Boolean(
      (process.env.INOKS_STORY_COVER_BASE_URL || process.env.INOKS_STORY_COVER_ENDPOINT)
      && (process.env.INOKS_STORY_COVER_API_KEY || keyFor("kkaiapi")),
    );
  ...
  app.get("/api/v1/project", async (c) => {
    ...
    try {
      ...
      // Check if language was explicitly set in inoks-story-webnovel.json (not just the schema default)
      raw = JSON.parse(await readFile(join(root, "inoks-story-webnovel.json"), "utf-8")) as Record<string, unknown>;
    } catch (error) {
      throw new ApiError(
        500,
        "PROJECT_CONFIG_INVALID",
        `Failed to load inoks-story-webnovel.json: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  ...
  app.get("/api/v1/project/input-governance-mode", async (c) => {
    const raw = JSON.parse(await readFile(join(root, "inoks-story-webnovel.json"), "utf-8"));
  ...
  app.get("/api/v1/books/:id/truth", async (c) => {
    ...
    const { isNewLayoutBook } = await import("@inoks-story-webnovel/core");
  ...
  app.get("/api/v1/project/prose-quality", async (c) => {
  ...
  app.get("/api/v1/books/:id/quality/prose/:chapter", async (c) => {
    ...
    const path = join(state.bookDir(id), "quality", "prose", `chapter-${String(chapter).padStart(4, "0")}.json`);
    const raw = await readFile(path, "utf-8").catch(() => "");
    return raw ? c.json(JSON.parse(raw)) : c.json({ error: "Quality report not found" }, 404);
  });

  app.get("/api/v1/books/:id/story-status", async (c) => {
    const id = c.req.param("id");
    if (!isSafeBookId(id)) return c.json({ error: "Invalid book id" }, 400);
    const bookDir = state.bookDir(id);
    const store = new ChapterCommitStore(bookDir);
    const [head, commits, projectionFailures, preflight] = await Promise.all([
      store.loadHead(),
      store.listCommits(),
  ...
  app.post("/api/v1/books/:id/story-repair", async (c) => {
    const id = c.req.param("id");
    if (!isSafeBookId(id)) return c.json({ error: "Invalid book id" }, 400);
    const bookDir = state.bookDir(id);
    const repair = await repairStorySystem(bookDir);
  ...
  app.get("/api/v1/books/:id/lock", async (c) => {
    ...
    return c.json({ ...lock, daemonRunning: schedulerInstance?.isRunning ?? false });
  });

  app.post("/api/v1/books/:id/force-unlock", async (c) => {
    const bookId = c.req.param("id");
    const body = await c.req.json().catch(() => ({})) as { confirm?: unknown; force?: unknown };
    if (body.confirm !== true) {
      throw new ApiError(400, "UNLOCK_CONFIRMATION_REQUIRED", "Confirmation is required");
    }
    try {
      const result = await state.forceReleaseBookLock(bookId, { force: body.force === true });
      broadcast("book:unlock", { bookId, removed: result.removed, force: body.force === true });
      return c.json(result);
    } catch (error) {
      throw new ApiError(409, "BOOK_UNLOCK_REJECTED", error instanceof Error ? error.message : String(error));
    }
  });
  ...
  app.get("/api/v1/logs", async (c) => {
    const logPath = join(root, "inoks-story-webnovel.log");
    try {
      ...
      const externalEdit = requestedIntent === "edit_artifact" || sessionKind === "edit"
        ? await tryHandleExternalChatEdit({
          root,
            ...
            applyChapterAmendment: async (targetBookId, chapterNumber, content) => {
              const pipeline = new PipelineRunner(await buildPipelineConfig({
                bookIdForSettings: targetBookId,
              }));
              await pipeline.applyManualChapterAmendment(targetBookId, chapterNumber, content);
            },
          })
        : null;
      if (externalEdit) {
  ...
  app.post("/api/v1/books/:id/audit/:chapter", async (c) => {
    ...
    try {
      ...
      const content = await readFile(join(chaptersDir, match), "utf-8");
      const currentConfig = await loadCurrentProjectConfig();
      const { ContinuityAuditor } = await import("@inoks-story-webnovel/core");
  ...
  app.get("/api/v1/genres/:id", async (c) => {
    ...
    try {
      const { readGenreProfile } = await import("@inoks-story-webnovel/core");
  ...
  app.post("/api/v1/genres/:id/copy", async (c) => {
    ...
    try {
      const { getBuiltinGenresDir } = await import("@inoks-story-webnovel/core");
  ...
  app.post("/api/v1/books/:id/detect/:chapter", async (c) => {
    ...
    try {
      ...
      const content = await readFile(join(chaptersDir, match), "utf-8");
      const { analyzeAITells } = await import("@inoks-story-webnovel/core");
  ...
  app.put("/api/v1/books/:id/truth/:file{.+}", async (c) => {
    ...
    if (CANONICAL_DERIVED_TRUTH_FILES.has(file) && await hasStorySystemHead(bookDir)) {
      return c.json({
        error: `${file} is a derived ChapterCommit projection. Change canon through a chapter amendment and replay projections.`,
      }, 409);
    }
    const { content } = await c.req.json<{ content: string }>();
  ...
  app.get("/api/v1/books/:id/detect/stats", async (c) => {
    const id = c.req.param("id");
    try {
      const { loadDetectionHistory, analyzeDetectionInsights } = await import("@inoks-story-webnovel/core");
      const bookDir = state.bookDir(id);
  ...
  app.post("/api/v1/style/analyze", async (c) => {
    ...
    try {
      const { analyzeStyle } = await import("@inoks-story-webnovel/core");
  ...
  app.post("/api/v1/books/:id/import/chapters", async (c) => {
    ...
    try {
      const { splitChapters } = await import("@inoks-story-webnovel/core");
  ...
  app.get("/api/v1/doctor", async (c) => {
    ...
    const { GLOBAL_ENV_PATH } = await import("@inoks-story-webnovel/core");

    const checks = {
      inoksStoryJson: existsSync(join(root, "inoks-story-webnovel.json")),
