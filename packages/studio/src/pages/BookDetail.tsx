...
export function BookDetail({
  ...
  const {
    data: storyStatus,
    error: storyStatusError,
    refetch: refetchStoryStatus,
  } = useApi<StoryStatusData>(`/books/${bookId}/story-status`);
  ...
  const [repairingStory, setRepairingStory] = useState(false);
  ...
  }, [bookId, refetch, refetchStoryStatus, sse.messages]);
  ...
  const handleRepairStorySystem = async () => {
    setRepairingStory(true);
    try {
      const result = await fetchJson<{ ok: boolean; preflight?: { errors?: ReadonlyArray<string> } }>(
        `/books/${bookId}/story-repair`,
        { method: "POST" },
      );
      await refetchStoryStatus();
      if (!result.ok) {
        alert(
          data?.book.language === "en"
            ? `Repair finished, but verification still reports: ${(result.preflight?.errors ?? []).join("; ")}`
            : `修复已执行，但验证仍有问题：${(result.preflight?.errors ?? []).join("；")}`,
        );
      }
    } catch (e) {
      alert(e instanceof Error ? e.message : "Story System repair failed");
    } finally {
      setRepairingStory(false);
    }
  };
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <ShieldCheck
                size={18}
                className={storyStatus?.preflight.passed ? "text-emerald-500" : "text-amber-500"}
              />
            ...
            <p className="mt-2 text-xs text-muted-foreground max-w-3xl">
              {book.language === "en"
                ? "Accepted ChapterCommits are canonical; truth files, temporal memory, summaries, hooks, and retrieval indexes are rebuildable projections."
                : "accepted ChapterCommit 是唯一正史；真相文件、时间记忆、摘要、伏笔与检索索引均为可重建投影。"}
            </p>
          </div>
          <button
            onClick={handleRepairStorySystem}
            disabled={repairingStory}
            className="flex items-center gap-2 px-4 py-2 text-xs font-bold bg-secondary/60 text-muted-foreground rounded-lg hover:text-foreground hover:bg-secondary transition-all border border-border/50 disabled:opacity-50"
          >
            <RefreshCw size={14} className={repairingStory ? "animate-spin" : ""} />
            {repairingStory
              ? t("common.loading")
              : book.language === "en" ? "Repair projections" : "修复投影"}
          </button>
        </div>
        {storyStatusError ? (
          <p className="mt-4 text-xs text-destructive">{storyStatusError}</p>
        ) : storyStatus ? (
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <StoryMetric
              label={book.language === "en" ? "Canonical chapter" : "正史章节"}
              value={String(storyStatus.headChapter)}
            />
            <StoryMetric
              label={book.language === "en" ? "Commit count" : "Commit 数"}
              value={String(storyStatus.commitCount)}
            />
            <StoryMetric
              label={book.language === "en" ? "Preflight" : "预检"}
              value={storyStatus.preflight.passed
                ? (book.language === "en" ? "Passed" : "通过")
                : (book.language === "en" ? "Blocked" : "阻断")}
              danger={!storyStatus.preflight.passed}
            />
            <StoryMetric
              label={book.language === "en" ? "Projection failures" : "投影失败"}
              value={String(storyStatus.projectionFailures.length)}
              danger={storyStatus.projectionFailures.length > 0}
            />
          </div>
        ) : (
          <div className="mt-4 h-16 rounded-xl bg-secondary/20 animate-pulse" />
        )}
        {storyStatus && (storyStatus.preflight.errors.length > 0 || storyStatus.preflight.warnings.length > 0) && (
          <div className="mt-4 rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-xs">
            {storyStatus.preflight.errors.map((message) => (
              <p key={`error-${message}`} className="text-destructive">{message}</p>
            ))}
            {storyStatus.preflight.warnings.map((message) => (
              <p key={`warning-${message}`} className="text-amber-600">{message}</p>
            ))}
          </div>
        )}
      </div>
...
function StoryMetric({
  ...
  return (
    <div className="rounded-xl border border-border/40 bg-secondary/20 px-4 py-3">
      ...
      <div className={`mt-1 text-lg font-mono font-semibold ${danger ? "text-destructive" : "text-foreground"}`}>
        {value}
      </div>
    </div>
  );
