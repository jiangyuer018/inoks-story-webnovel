      ...
      {data.proseQuality && (
        <section className="rounded-2xl border border-border/50 bg-card/70 p-4 shadow-sm">
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm">
            <span className="font-semibold">{t("nav.connected") === "已连接" ? "正文质量" : "Prose quality"}</span>
            <span>{t("nav.connected") === "已连接" ? "分数" : "Score"}: {data.proseQuality.score}</span>
            <span>{t("nav.connected") === "已连接" ? "等级" : "Level"}: {data.proseQuality.level}</span>
            <span>blocking: {data.proseQuality.blockingCount}</span>
            <span>advisory: {data.proseQuality.advisoryCount}</span>
            <span>{data.proseQuality.repaired
              ? (t("nav.connected") === "已连接" ? `已自动修复 ${data.proseQuality.iterations} 轮` : `Auto-repaired ${data.proseQuality.iterations} round(s)`)
              : (t("nav.connected") === "已连接" ? "未触发自动修复" : "No automatic repair")}</span>
            <a
              className="font-medium text-primary hover:underline"
              href={`/api/v1/books/${encodeURIComponent(bookId)}/quality/prose/${chapterNumber}`}
              target="_blank"
              rel="noreferrer"
            >
              {t("nav.connected") === "已连接" ? "查看报告" : "View report"}
            </a>
          </div>
        </section>
      )}
