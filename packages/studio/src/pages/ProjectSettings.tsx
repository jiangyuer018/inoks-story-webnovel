...
import { Bell, Bot, Database, FileText, MessageSquare, Radar, RotateCcw, Search, Settings2, ShieldCheck, Plus, Trash2 } from "lucide-react";
...
interface ProseQualitySettings {
  enabled: boolean;
  enforcement: "strict" | "balanced" | "report-only";
  autoRepair: boolean;
  maxRepairIterations: number;
  minimumScore: number;
  failOnUnresolvedBlocking: boolean;
  saveRejectedDraft: boolean;
  maxAutomaticModificationRatio: number;
  applyTo: Array<"chapter" | "short-fiction" | "continuation" | "revision">;
}

interface LongFormMemorySettings {
  enabled: boolean;
  authority: "chapter-commit";
  strictPreflight: boolean;
  blockOnProjectionFailure: boolean;
  ...
  retrieval: {
    ...
    retrievedTokenRatio: number;
    compressedTokenRatio: number;
  };
}
      ...
      {proseQuality && longFormMemory && (
        <>
          ...
          <SettingsCard
            ...
            description={isZh
              ? "所有中文章节、续写、修订和短篇在正式保存前都会经过同一个确定性扫描与安全修订门。"
              : "Every Chinese chapter, continuation, revision, and short passes the same deterministic gate before formal persistence."}
          ...
          >
            <div className="grid gap-4 md:grid-cols-2">
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={proseQuality.enabled}
                  onChange={(event) => setProseQuality({ ...proseQuality, enabled: event.target.checked })}
                />
                {isZh ? "启用质量门（默认启用）" : "Enable quality gate (default)"}
              </label>
              <label className="space-y-1 text-sm">
                <span>{isZh ? "执行模式" : "Enforcement"}</span>
                <select
                  className={fieldClass}
                  value={proseQuality.enforcement}
                  onChange={(event) => setProseQuality({
                    ...proseQuality,
                    enforcement: event.target.value as ProseQualitySettings["enforcement"],
                  })}
                >
                  <option value="strict">{isZh ? "strict：未解决阻断项不落盘" : "strict: block persistence"}</option>
                  <option value="balanced">{isZh ? "balanced：自动修复，剩余项告警" : "balanced: repair, then warn"}</option>
                  <option value="report-only">{isZh ? "report-only：只扫描和报告" : "report-only: scan only"}</option>
                </select>
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={proseQuality.autoRepair}
                  onChange={(event) => setProseQuality({ ...proseQuality, autoRepair: event.target.checked })}
                />
                {isZh ? "自动自然化修订" : "Automatic naturalization"}
              </label>
              <label className="space-y-1 text-sm">
                <span>{isZh ? "最低分数" : "Minimum score"}</span>
                <input
                  className={fieldClass}
                  type="number"
                  min={0}
                  max={100}
                  value={proseQuality.minimumScore}
                  onChange={(event) => setProseQuality({ ...proseQuality, minimumScore: Number(event.target.value) })}
                />
              </label>
            </div>
          </SettingsCard>
          ...
          <SettingsCard
            title={isZh ? "长篇记忆与正史提交" : "Long-form Memory & Canon"}
            description={isZh
              ? "accepted ChapterCommit 是唯一正史入口；Markdown、MemoryDB、摘要、伏笔和检索索引都是可重放投影。"
              : "Accepted ChapterCommit is the sole canon entry; Markdown, MemoryDB, summaries, hooks, and indexes are replayable projections."}
          ...
          >
            <div className="grid gap-4 md:grid-cols-2">
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={longFormMemory.enabled}
                  onChange={(event) => setLongFormMemory({ ...longFormMemory, enabled: event.target.checked })}
                />
                {isZh ? "启用 ChapterCommit 权威链" : "Enable ChapterCommit authority"}
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={longFormMemory.strictPreflight}
                  onChange={(event) => setLongFormMemory({ ...longFormMemory, strictPreflight: event.target.checked })}
                />
                {isZh ? "写章前严格预检" : "Strict preflight before writing"}
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={longFormMemory.blockOnProjectionFailure}
                  onChange={(event) => setLongFormMemory({ ...longFormMemory, blockOnProjectionFailure: event.target.checked })}
                />
                {isZh ? "必需投影失败时阻止下一章" : "Block next chapter on required projection failure"}
              </label>
              <label className="space-y-1 text-sm">
                <span>{isZh ? "序列摘要章数" : "Sequence summary size"}</span>
                <input
                  className={fieldClass}
                  type="number"
                  min={5}
                  max={10}
                  value={longFormMemory.sequenceSize}
                  onChange={(event) => setLongFormMemory({ ...longFormMemory, sequenceSize: Number(event.target.value) })}
                />
              </label>
            </div>
            <button
              ...
              className={`rounded-lg px-4 py-2 text-sm font-bold ${c.btnPrimary} disabled:opacity-40`}
            >
              {saving === "prose-quality" ? t("config.saving") : t("config.save")}
            </button>
          </SettingsCard>
        </>
      )}
            ...
            <div className="flex items-center justify-between gap-2">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {isZh ? "这些文件会保存到 .inoks-story-webnovel/skills/<id>/SKILL.md。" : "Saved to .inoks-story-webnovel/skills/<id>/SKILL.md."}
                </p>
              </div>
              {editingSkillId ? (
                <button
