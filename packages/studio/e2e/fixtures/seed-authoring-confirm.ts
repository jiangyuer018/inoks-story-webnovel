 ...
 * confirm E2E. The dev server uses test-project/ as its INOKS_STORY_PROJECT_ROOT.
 * We create:
 ...
 *  - test-project/.inoks-story-webnovel/secrets.json
 ...
 *
 * IMPORTANT: we do NOT touch test-project/inoks-story-webnovel.json — that file is shared
 * across all E2E tests and must not be overwritten.
 *
 * The actual LLM calls are bypassed by INOKS_STORY_AGENT_LLM_STUB=1 set in
...
export async function seedAuthoringConfirm(): Promise<void> {
  ...
  const sessionsDir = resolve(E2E_ROOT, ".inoks-story-webnovel", "sessions");
  try {
  ...
  // The INOKS_STORY_AGENT_LLM_STUB=1 env var ensures no real API call is made.
