/**
 * Verify that source manifests will become publishable registry manifests.
 *
 * Usage:
 *   node scripts/verify-no-workspace-protocol.mjs packages/cli packages/core
 *   node ../../scripts/verify-no-workspace-protocol.mjs .
 *
 * A local workspace may use workspace: specifiers to avoid resolving its own
 * unpublished packages from npm. They are allowed only when the package's
 * prepack hook is the shared manifest transformer, which replaces them with
 * real versions before npm pack/publish.
 */
  ...
  for (const field of ["dependencies", "optionalDependencies", "peerDependencies"]) {
    ...
    for (const [name, specifier] of Object.entries(deps)) {
      ...
      if (specifier.startsWith("workspace:")) {
        const convertsWorkspaceDependencies =
          typeof pkg.scripts?.prepack === "string"
          && pkg.scripts.prepack.includes("prepare-package-for-publish.mjs");
        if (convertsWorkspaceDependencies) {
          continue;
        }
