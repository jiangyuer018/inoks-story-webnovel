...
- 修复故事控制中心把“尚无质量报告”误判为质量阻断、读取最旧而非最新报告的问题；新书现在直接引导生成第一章
- Studio 新增旧章节迁移预览和二次确认入口：确认后先备份章节与 story 目录，再建立 ChapterCommit 链并重放投影
- “下一步”组件改用现有 Studio 状态色和按钮组件，按真实的旧历史、质量阻断、Commit 与投影状态给出可执行动作

### 正文质量与超长篇记忆

- 新增原生中文 `ProseQualityGate`：确定性 blocking/advisory 扫描、项目与书籍白名单、最多两轮自然化修订、连续性/删除比例/文本差异回退、结构化报告和严格模式拒稿目录
- Writer、Reviser、短篇和所有正式章节入口统一注入核心正文约束；干净正文保持幂等且不调用修订模型
- 新增以 accepted `ChapterCommit` 为唯一正史入口的 Story System：确定性 Commit/事件 ID、哈希链、源码快照、崩溃可恢复事务、事件日志和可重放投影
- 扩展现有 MemoryDB 为带 Commit/Event 来源的时间事实投影，并新增当前事实优先的分层长篇检索、活跃伏笔、层级摘要、实体/关系与检索索引
...
- CLI 新增 `inoks-story forecast create / show / select`，与 Core 的推演 schema、存储、上下文指纹、agent 和 runner 共用同一套非正史边界
...
- CLI 新增 `inoks-story translate init / run / export`；Studio Chat 也可通过确认动作创建翻译项目，不要求用户记忆 `zh`、`en` 等语言缩写
...
- CLI 遵守单本书的 `writing.reviewMode`；新增 `inoks-story auto [book-id] <目标章号>` 连续写作到指定章节（#307）
...
- 修复 `@inoks-story-webnovel/cli@1.6.2` / `@inoks-story-webnovel/studio@1.6.2` 发布到 npm 时 registry manifest 泄漏 `workspace:*` 的问题；Windows / npm 全局升级请直接安装 `1.6.3` 或更新到 `latest`
...
互动影游与 Skill 系统大版本：把 Inoks Story Webnovel 的创作入口从“小说 + Play”继续扩展到互动影游、剧本、分镜和可插拔专业能力。Studio Chat 现在可以按用户意图调用外部 / 内置 skills，也能在需要真实资料时生成可追溯研究报告，同时修复几类影响长任务继续推进和用户协作编辑的稳定性问题。
...
- 研究报告保存为 `.inoks-story-webnovel/research/` 下的参考材料，不直接污染 story truth、角色卡或正文

## v1.5.0

### Release Focus

Inoks Story Webnovel Play 与创作工作台大版本：把 Inoks Story Webnovel 从“自动写下一章”的管线工具推进到更完整的 Story Creation AI Agent。长篇、短篇、同人、番外、仿写、续写、封面和开放世界互动开始共享同一套 Studio Chat / CLI / TUI 交互内核，并围绕指令遵循、上下文管理和可视化体验做了系统性整理。

### Major Features

- 新增 **Inoks Story Webnovel Play** 开放世界 / 分支互动入口：支持自由动作、可点击选择、世界契约、非固定时间推进、角色 agent、物品 / 证据 / 关系状态、HUD 和自动配图
...
- 模型配置、封面服务、聚合 API 入口和错误提示进一步区分：Inoks Story Webnovel 执行错误、模型供应商错误和图片生成错误不再混在一起
...
- 长篇章节写作的自动审稿修稿轮数新增 `writing.reviewRetries` 项目配置，默认仍为 1；需要更强修稿时可执行 `inoks-story config set writing.reviewRetries 3`
...
- Chat 可直接编辑项目内生成文本产物，适合调整章节、封面提示词、简介等文件后再继续使用 Inoks Story Webnovel 写作链路
...
- **Windows Studio 启动修复**：`inoks-story studio` 在 Windows 下不再因绝对路径 loader 被当成非法 ESM URL 而崩溃
- **Bootstrap 项目回退到 env 配置**：空目录 auto-init 后的 Studio 项目，在未配置服务时会回退到全局 `.inoks-story-webnovel/.env`，`book create` 不再先死在缺 key
...
- **空目录直接启动**：`inoks-story` / `inoks-story studio` 现在会自动初始化最小项目骨架并启动 Studio，不再要求显式先跑 `init`
...
Studio 2.0 正式发布。`inoks-story` 现在默认直接启动 Studio，本地 Web 工作台成为主入口；TUI 保留为 `inoks-story tui`。

### 新功能

- **Studio 2.0 默认入口**：`inoks-story` 直接启动 Studio，首页、服务商管理、写作工作台统一为新的主交互入口
- **自定义 OpenAI-compatible 服务**：Studio 现支持自定义 `baseUrl`、协议类型（`chat` / `responses`）与流式开关，兼容更多中转站和聚合网关
- **配置来源切换**：Studio 新增 `.env` 与 Studio 配置的显式切换，不再只能被目录里的 `INOKS_STORY_LLM_*` 被动覆盖
...
- 修复 `inoks-story` 启动 Studio 时因未设置默认模型而直接抛出 `llm.model` 校验错误
...
统一交互内核——TUI、Studio、`inoks-story interact`、OpenClaw Skill 共享同一套自然语言理解和执行运行时。

### 新功能

- **共享交互运行时**（`packages/core/src/interaction/`）：自然语言路由器（15+ intent）、会话管理、编辑事务控制器、事件追踪、阶段遥测
- **Ink TUI 仪表盘**：`inoks-story` 直接进入全屏 Ink + React 仪表盘，对话式创作，slash 命令 Tab 补全，主题动效（writing/auditing/revising/planning 各有独立动画），i18n 中英双语
...
- **`inoks-story interact --json`**：共享交互 JSON 入口，返回 request / response / session / events，供 OpenClaw 和外部 Agent 直接调用
...
- **Review Reject 回滚**：`inoks-story review reject` 回滚 state 到被拒章节之前的快照，丢弃下游章节和记忆索引
- **State Validation Recovery**：state 校验失败自动重试 settler，仍失败则降级保存，支持 `inoks-story write repair-state` 手动修复
...
- **自定义 HTTP Headers**：`INOKS_STORY_LLM_HEADERS` 环境变量注入自定义 HTTP 头
...
Inoks Story Webnovel Studio + 稳定性加固。从 CLI 工具升级为 CLI + Web 工作台。

### Inoks Story Webnovel Studio

- `inoks-story studio` 启动本地 Web 工作台（Vite + React + Hono，默认端口 4567）
...
- 新增 `inoks-story eval` 命令 — 结构化质量评估报告
...
- 用户 `INOKS_STORY_LLM_MAX_TOKENS` 作为全局上限生效（#87）
...
- `inoks-story book delete <id>`：删除书籍及全部数据（`--force` 跳过确认）
- `inoks-story status --chapters`：显示每章状态和 failed 章节的 critical issues
- 审计 JSON 解析容错（#51）
...
- `inoks-story up` 自动写入 `inoks-story-webnovel.log`，守护进程重启后可追溯
...
- `inoks-story doctor` 失败时给出针对性 hints（检查 baseUrl、试 stream:false、检查 API Key）
...
把已有的小说（单文件或章节目录）导入 Inoks Story Webnovel，系统自动拆章、逆向工程生成全套真相文件（世界状态、伏笔、角色矩阵等），之后直接 `write next` 续写。

```bash
inoks-story import chapters 我的小说 --from 已有章节/        # 从目录导入
inoks-story import chapters 我的小说 --from 全书.txt          # 从单文件导入（自动按"第X章"拆分）
inoks-story import chapters 我的小说 --from 全书.txt --split "Chapter\\s+\\d+"  # 自定义分章正则
inoks-story write next 我的小说                               # 无缝续写
```
...
```bash
inoks-story import canon 烈焰前传 --from 吞天魔帝   # 导入正传正典到番外
inoks-story write next 烈焰前传                     # 写手自动读取正典约束
```
...
```bash
inoks-story style analyze 参考小说.txt                     # 分析：句长、TTR、修辞特征
inoks-story style import 参考小说.txt 吞天魔帝 --name 某作者  # 导入文风到书
```
...
```bash
inoks-story config set-model writer gpt-4o-mini                                    # 简单模型覆盖
...
inoks-story config show-models                                                      # 查看路由全景
```
...
```bash
inoks-story analytics 吞天魔帝          # 审计通过率、高频问题类别、问题最多的章节
inoks-story analytics 吞天魔帝 --json   # 结构化输出
```
...
- `inoks-story export` 支持 `--format md`、`--output <path>`、`--approved-only`
...
| AIGC 检测 API | 外部 API 集成（GPTZero / Originality / 自定义端点），`inoks-story detect` 命令 |
| 文风指纹学习 | 从参考文本提取 StyleProfile（句长、TTR、修辞特征），注入 Writer prompt |
| 反检测改写 | ReviserAgent `anti-detect` 模式，检测→改写→重检测循环 |
| 检测反馈闭环 | `detection_history.json` 记录每次检测/改写结果，`inoks-story detect --stats` 查看统计 |

```bash
inoks-story style analyze reference.txt         # 分析参考文本文风
inoks-story style import reference.txt 吞天魔帝  # 导入文风到书
inoks-story detect 吞天魔帝 --all               # 全书 AIGC 检测
inoks-story detect --stats                      # 检测统计
```
...
```bash
inoks-story genre list                      # 查看所有题材
inoks-story genre show xuanhuan             # 查看玄幻的完整规则
inoks-story genre copy xuanhuan             # 复制到项目中，随意改
inoks-story genre create wuxia --name 武侠   # 从零创建新题材
```
...
- `inoks-story update` 一键更新、`inoks-story init` 支持当前目录初始化
- API 错误附带中文诊断提示，`inoks-story doctor` 含 API 连通性测试
