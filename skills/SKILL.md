---
name: inoks-story
description: Story Creation and Translation AI Agent with Studio Chat, CLI, and TUI - use for long-form novels, short fiction, scripts, storyboards, interactive-film projects, open-world / branching play, fan fiction, spinoffs, style imitation, continuations, covers, and multilingual EPUB/PDF/TXT/Markdown translation. Includes runtime skills, traceable research, governed context, persistent story state, multi-model routing, image services, and Inoks Story Webnovel Studio.
version: 2.7.0
metadata: { "openclaw": { "emoji": "📖", "requires": { "bins": ["inoks-story", "node"], "env": ["OPENAI_API_KEY"] }, "primaryEnv": "OPENAI_API_KEY", "homepage": "https://github.com/jiangyuer018/inoks-story-webnovel", "install": [{ "id": "npm", "kind": "node", "package": "@inoks-story-webnovel/cli", "label": "Install Inoks Story Webnovel (npm)" }] } }
---

# Inoks Story Webnovel - Story Creation AI Agent

Inoks Story Webnovel is a story creation and multilingual translation AI agent for long-form novels, short fiction, scripts, storyboards, interactive-film projects, fan works, continuations, covers, open-world / branching interactive play, and long-document localization. Prefer the Studio Chat / action-surface workflow for natural-language requests: the model proposes or invokes typed actions, Inoks Story Webnovel executes them, and completion is derived from real tool results and files, not from prose claims.
...
Treat Inoks Story Webnovel as a confirmable action system, not a bag of prompt shortcuts. v1.7.1 adds non-canonical narrative forecasting, background production tasks that do not block discussion, retryable task state, whole-book backup / restore, and rollback-safe latest-chapter deletion.

- Natural-language requests should go through Studio Chat / TUI / `inoks-story interact` whenever possible.
...
## When to Use Inoks Story Webnovel
...
inoks-story init my-writing-project
...
inoks-story config set-global --provider openai --base-url https://api.openai.com/v1 --api-key-env OPENAI_API_KEY --model gpt-4o
# For compatible/proxy endpoints, use --provider custom and point ONLY to trusted endpoints:
# inoks-story config set-global --provider custom --base-url https://your-trusted-proxy.com/v1 --api-key-env OPENAI_API_KEY --model gpt-4o
```

### Multi-Model Routing (Optional)
```bash
# Assign different models to different agents — balance quality and cost
inoks-story config set-model writer claude-sonnet-4-20250514 --provider anthropic --base-url https://api.anthropic.com --api-key-env ANTHROPIC_API_KEY
inoks-story config set-model auditor gpt-4o --provider openai
inoks-story config show-models
```
Agents without explicit overrides fall back to the global model.

### Provider setup tip

When the user needs an aggregator provider, point them to Studio's service settings. Inoks Story Webnovel includes kkaiapi and OpenRouter as optional aggregator choices. For kkaiapi, use:
...
1. **Initialize and create book**:
   ```bash
   inoks-story book create --title "My Novel Title" --genre xuanhuan --chapter-words 3000
   # Or with a creative brief (your worldbuilding doc / ideas):
   inoks-story book create --title "My Novel Title" --genre xuanhuan --chapter-words 3000 --brief my-ideas.md
   ```
...
2. **Generate initial chapters** (e.g., 5 chapters):
   ```bash
   inoks-story write next book-id --count 5 --words 3000 --context "young protagonist discovering powers"
   ```
...
3. **Review and approve chapters**:
   ```bash
   inoks-story review list book-id
   inoks-story review approve-all book-id
   ```

4. **Export the book** (supports txt, md, epub):
   ```bash
   inoks-story export book-id
   inoks-story export book-id --format epub
   ```
...
1. **List your books**:
   ```bash
   inoks-story book list
   ```

2. **Continue from last chapter**:
   ```bash
   inoks-story write next book-id --count 3 --words 2500 --context "protagonist faces critical choice"
   ```
   - Inoks Story Webnovel maintains structured runtime state, readable Markdown projections, control docs, and temporal memory for consistency
   - If only one book exists, omit `book-id` for auto-detection

3. **Review and approve**:
   ```bash
   inoks-story review approve-all
   ```

### Workflow 2.5: Shared Natural-Language Control (Recommended For OpenClaw)

When Inoks Story Webnovel is being driven by OpenClaw or another external agent, prefer the shared interaction executor instead of stitching together many ad-hoc CLI calls:

```bash
inoks-story interact --json --message "continue the current book, but keep the pacing tighter"
inoks-story interact --json --message "rewrite chapter 3"
inoks-story interact --json --message "start an open-world detective game about a locked archive room"
```
...
1. Create isolated candidates from the current book state:
   ```bash
   inoks-story forecast create book-id --divergence "Does the protagonist reveal the evidence now?" --branches 3 --horizon 5
   ```
2. Compare the branch cards in Studio Chat, or inspect the saved comparison with `inoks-story forecast show [book-id] <forecast-id>`.
...
4. Select one candidate with the Studio card or:
   ```bash
   inoks-story forecast select book-id forecast-id branch-2
   ```
...
Use this when you have an existing novel (or partial novel) and want Inoks Story Webnovel to pick up where it left off.

1. **Import from a single text file** (auto-splits by chapter headings):
   ```bash
   inoks-story import chapters book-id --from novel.txt
   ```
...
2. **Import from a directory** of separate chapter files:
   ```bash
   inoks-story import chapters book-id --from ./chapters/
   ```
   - Reads `.md` and `.txt` files in sorted order

3. **Resume interrupted import**:
   ```bash
   inoks-story import chapters book-id --from novel.txt --resume-from 15
   ```

4. **Continue writing** from the imported chapters:
   ```bash
   inoks-story write next book-id --count 3
   ```
   - Inoks Story Webnovel rebuilds structured state, chapter summaries, hooks, character relationships, readable projections, and a style guide from the imported chapters
...
1. **Analyze reference text**:
   ```bash
   inoks-story style analyze reference_text.txt
   ```
   - Examines vocabulary, sentence structure, tone, pacing

2. **Import style to your book**:
   ```bash
   inoks-story style import reference_text.txt book-id --name "Author Name"
   ```
...
1. **Import parent canon**:
   ```bash
   inoks-story import canon spinoff-book-id --from parent-book-id
   ```
...
2. **Continue spinoff**:
   ```bash
   inoks-story write next spinoff-book-id --count 3 --context "alternate timeline after Chapter 20"
   ```
...
1. **Generate draft only**:
   ```bash
   inoks-story draft book-id --words 3000 --context "protagonist escapes" --json
   ```

2. **Audit the chapter** (33-dimension quality check):
   ```bash
   inoks-story audit book-id chapter-1 --json
   ```
...
3. **Revise with specific mode**:
   ```bash
   inoks-story revise book-id chapter-1 --mode polish --json
   ```
...
inoks-story detect book-id

# Deep scan all chapters
inoks-story detect book-id --all
```
...
```bash
inoks-story analytics book-id --json
# Shorthand alias
inoks-story stats book-id --json
```
...
inoks-story book create --title "The Last Delver" --genre litrpg --chapter-words 3000

# Or set language explicitly
inoks-story book create --title "My Novel" --genre other --lang en

# Set English as default for all projects
inoks-story config set-global --lang en
```
...
- Use `inoks-story genre list` to see all available genres

### Workflow 11: Fan Fiction

```bash
# Create a fanfic from source material
inoks-story fanfic init --title "My Fanfic" --from source-novel.txt --mode canon

# Modes: canon (faithful), au (alternate universe), ooc (out of character), cp (ship-focused)
inoks-story fanfic init --title "What If" --from source.txt --mode au --genre other
```
...
Use Studio Chat or `inoks-story interact` for small, explicit edits to generated text artifacts. Treat the chat layer like an external editor: it may edit persisted files, then the writing pipeline can continue from the updated state.

```bash
inoks-story interact --book my-book --json --message "把第 3 章里那句过长的解释删短一点，但不要改剧情事实"
inoks-story interact --json --message "把 covers/demo/cover-prompt.md 里的人物拉近一点，标题字更大"
```
...
When the user asks for a rename or targeted replacement, keep the request explicit and scoped. Prefer Studio Chat / `inoks-story interact` over hidden slash shortcuts; the agent should route the edit through the shared action surface and report which file changed.
...
- Shares the same interaction kernel as `inoks-story interact` and Studio
...
```text
.inoks-story-webnovel/skills/<skill-id>/SKILL.md
```

External skill directories can be loaded with:

```bash
export INOKS_STORY_SKILL_DIRS=/abs/path/to/skills
```

Guidelines for agent orchestration:
- Let Inoks Story Webnovel auto-select skills when the user gives a natural request such as "make this a detective evidence-chain open world".
...
- Do not treat skills as permissions. File edits, book creation, chapter writing, image generation, and exports still require the normal Inoks Story Webnovel tools and confirmation gates.
...
In Studio Chat, these actions should be proposed with a confirmation card first. After confirmation, Inoks Story Webnovel writes the artifact and reports the saved files. Do not hand-write a fake "created" result in prose.
...
```bash
inoks-story translate init --from ./source.epub --source Chinese --target English
inoks-story translate run <project-id>
inoks-story translate export <project-id> --format epub
```
...
- For scanned PDFs without a text layer, run OCR before importing; Inoks Story Webnovel currently reads text-based PDFs.

## Inoks Story Webnovel Studio (Web Workbench)

`inoks-story studio` launches a local web UI (default port 4567) that provides a visual interface for all Inoks Story Webnovel operations:
...
- **Runtime Skill management** — list built-in skills, add project-local skills, edit `.inoks-story-webnovel/skills/<id>/SKILL.md`, and force skills from Chat
...
```bash
inoks-story studio              # Start on default port 4567
inoks-story studio -p 8080      # Start on custom port
```
...
```bash
inoks-story agent "写一部都市题材的小说，主角是一个年轻律师，第一章三千字"
```
...
These are the preferred tools when Inoks Story Webnovel is driven by OpenClaw, Studio chat, or `inoks-story agent`:
...
```bash
# Explicit
inoks-story write next book-123 --count 1

# Auto-detected (if only one book exists)
inoks-story write next --count 1
```

### --json Flag
All content-generating commands support `--json` for structured output. Essential for programmatic use:
```bash
inoks-story draft book-id --words 3000 --context "guidance" --json
```

### Long-Term Memory
Inoks Story Webnovel maintains each book's memory in three layers:
...
Agents use the governed composer to select the relevant parts for the current task instead of blindly injecting every file. During `import chapters`, Inoks Story Webnovel rebuilds structured state and projections from existing content via the ChapterAnalyzerAgent.
...
```bash
inoks-story write next book-id --count 2 --context "protagonist discovers betrayal, must decide whether to trust mentor"
```
...
```bash
inoks-story genre list
inoks-story genre show xuanhuan
```

### Create Custom Genre
```bash
inoks-story genre create my-genre --name "My Genre"
# Options: --numerical, --power, --era
inoks-story genre create dark-xuanhuan --name "Dark Xuanhuan" --numerical --power
```

### Copy Built-in Genre for Customization
```bash
inoks-story genre copy xuanhuan
...
|---------|---------|-------|
| `inoks-story init [name]` | Initialize project | One-time setup |
| `inoks-story book create` | Create new book | Returns book-id. `--brief <file>`, `--lang en/zh`, `--genre litrpg/progression/...` |
| `inoks-story book list` | List all books | Shows IDs, statuses |
| `inoks-story write next` | Full pipeline (draft→audit→revise) | Primary workflow command |
| `inoks-story draft` | Generate draft only | No auditing/revision |
| `inoks-story audit` | 33-dimension quality check | Standalone evaluation |
| `inoks-story revise` | Revise chapter | Modes: polish/spot-fix/rewrite/rework/anti-detect |
| `inoks-story agent` | Natural language interface | Flexible requests |
| `inoks-story style analyze` | Analyze reference text | Extracts style profile |
| `inoks-story style import` | Apply style to book | Makes style permanent |
| `inoks-story import canon` | Link spinoff to parent | For prequels/sequels |
| `inoks-story import chapters` | Import existing chapters | Reverse-engineers truth files for continuation |
| `inoks-story detect` | AIGC detection | Flags AI-generated passages |
| `inoks-story export` | Export finished book | Formats: txt, md, epub |
| `inoks-story analytics` / `inoks-story stats` | View book statistics | Word count, audit rates, token usage |
| `inoks-story radar scan` | Platform trend analysis | Informs new book ideas |
| `inoks-story short run` | Generate standalone short fiction | Outputs manuscript, sales package, cover prompt, optional cover |
| `inoks-story config set-global` | Configure LLM provider | OpenAI/Anthropic/custom (any OpenAI-compatible) |
| `inoks-story config set-model <agent> <model>` | Set model override for a specific agent | `--provider`, `--base-url`, `--api-key-env` for multi-provider routing |
| `inoks-story config show-models` | Show current model routing | View per-agent model assignments |
| `inoks-story doctor` | Diagnose issues | Check installation |
| `inoks-story update` | Update to latest version | Self-update |
| `inoks-story up/down` | Daemon mode | Background processing. Logs to `inoks-story-webnovel.log` (JSON Lines). `-q` for quiet mode |
| `inoks-story review list/approve-all` | Manage chapter approvals | Quality gate |
| `inoks-story fanfic init` | Create fanfic from source material | `--from <file>`, `--mode canon/au/ooc/cp` |
| `inoks-story genre list` | List all available genres | Shows English and Chinese genres with default language |
| `inoks-story genre create <id>` | Create custom genre profile | `--name`, `--numerical`, `--power`, `--era` |
| `inoks-story genre copy <id>` | Copy built-in genre to project | For customization |
| `inoks-story write rewrite <book> <ch>` | Rewrite a specific chapter | Deletes chapter and later, rewrites from that point |
| `inoks-story book update [book-id]` | Update book settings | `--chapter-words`, `--target-chapters`, `--status`, `--lang` |
| `inoks-story book delete <book-id>` | Delete book and all chapters | `--force` to skip confirmation |
| `inoks-story plan chapter [book-id]` | Generate chapter intent | Preview what next chapter will do before writing |
| `inoks-story compose chapter [book-id]` | Generate runtime artifacts | Context, rule-stack, trace for next chapter |
| `inoks-story consolidate [book-id]` | Consolidate chapter summaries | Reduces context for long books (volume-level summaries) |
| `inoks-story forecast create/show/select` | Compare non-canonical future branches | Selection saves a candidate plan only; canon remains unchanged |
| `inoks-story eval [book-id]` | Quality evaluation report | `--json`, `--chapters <range>`. Composite quality score |
| `inoks-story` / `inoks-story studio` | Start web workbench | `-p` for port. Local web UI for book management |
| `inoks-story fanfic show [book-id]` | Display parsed fanfic canon | Shows imported source material analysis |
| `inoks-story fanfic refresh [book-id]` | Re-import and regenerate fanfic canon | `--from <file>` for updated source material |
| `inoks-story interact` | Shared interaction endpoint | `--json`, `--message`, `--book`. Primary entry for OpenClaw |
| `inoks-story tui` | Launch TUI dashboard | Full-screen Ink + React interactive dashboard |
...
**"book-id not found"**
- Verify the ID with `inoks-story book list`
- Ensure you're in the correct project directory

**"Provider not configured"**
- Run `inoks-story config set-global` with valid credentials
- Check API key and base URL with `inoks-story doctor`
...
- Try `inoks-story revise` with `--mode rewrite`
...
2. **Start with style**: If imitating an author, run `inoks-story style import` before generation
3. **Import first**: For existing novels, use `inoks-story import chapters` to bootstrap truth files before continuing
4. **Review regularly**: Use `inoks-story review` to catch issues early
5. **Monitor audits**: Check `inoks-story audit` metrics to understand quality bottlenecks
...
8. **Check analytics**: Use `inoks-story analytics` to track quality trends over time
9. **Export frequently**: Keep backups with `inoks-story export`

## Security & Trust

- **License**: the ClawHub skill descriptor is MIT-0 per platform policy, but the underlying `@inoks-story-webnovel/cli`, `@inoks-story-webnovel/core`, and `@inoks-story-webnovel/studio` npm packages are **AGPL-3.0-only**. Running Inoks Story Webnovel and distributing modified versions are governed by AGPL. Full source on GitHub for auditability.
- **No install hooks**: npm package has no `preinstall`/`postinstall`/`install` scripts. Install is inert.
- **Local-only file I/O**: all read/write stays inside the project directory (`books/*`, `inoks-story-webnovel.json`, `inoks-story-webnovel.log`). No writes outside the working directory.
- **No telemetry**: Inoks Story Webnovel does not phone home, collect usage stats, or ship any data to Inoks Story Webnovel-controlled servers. The only outbound traffic is to the LLM provider endpoint you explicitly configure.
- **Credential handling**: prefer Studio service settings or `--api-key-env <VAR_NAME>` over literal keys. Studio stores service secrets in project-local `.inoks-story-webnovel/secrets.json`; CLI env settings live in `~/.inoks-story-webnovel/.env` or project `.env`. Treat all of these as secrets and keep them out of commits.
...
- **No elevated privileges**: Inoks Story Webnovel requires no sudo, no global state mutation, no network listening port (Studio binds `localhost:4567` only).

## Support & Resources

- **Homepage**: https://github.com/jiangyuer018/inoks-story-webnovel
- **Configuration**: Stored in project root after `inoks-story init`
- **Truth files**: Located in `books/<id>/story/` per book, with structured JSON in `story/state/`
- **Logs**: Check output of `inoks-story doctor` for troubleshooting
