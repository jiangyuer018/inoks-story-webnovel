...
- Unified core prose constraints and post-write scanning across Writer, Reviser, Short Fiction, and official chapter finalization; clean prose remains idempotent and does not call the revision model.
- Added a Story System where accepted `ChapterCommit` records are the sole canon write path, with deterministic Commit/event IDs, hash chains, source snapshots, crash-recoverable transactions, event logs, and replayable projections.
- Extended the existing MemoryDB into a Commit/Event-provenanced temporal projection, plus current-fact-first layered retrieval, active hooks, hierarchical summaries, entity/relationship indexes, and retrieval indexes.
...
- Added `inoks-story forecast create / show / select` to the CLI, sharing the same forecast schema, store, context fingerprints, agent, runner, and non-canonical boundary as Core
...
- Added `inoks-story translate init / run / export` to the CLI. Studio Chat can also propose a confirmed translation action without requiring users to know language codes such as `zh` or `en`
...
- The CLI now respects per-book `writing.reviewMode`; new `inoks-story auto [book-id] <target-chapter>` writes continuously to a target chapter (#307)
...
- Fixed `@inoks-story-webnovel/cli@1.6.2` / `@inoks-story-webnovel/studio@1.6.2` leaking `workspace:*` into the registry manifest when published to npm; for Windows / npm global upgrades, install `1.6.3` directly or update to `latest`
...
Major interactive film and Skill system release: extends Inoks Story Webnovel creation entries from "novels + Play" to interactive film/games, scripts, storyboards, and pluggable professional capabilities. Studio Chat can now invoke external / built-in skills based on user intent, and generate traceable research reports when real facts are needed; this release also fixes several stability issues affecting long-task continuation and collaborative user editing.
...
- Research reports are saved as reference material under `.inoks-story-webnovel/research/`, and do not directly contaminate story truth, character cards, or prose

## v1.5.0

### Release Focus

Major Inoks Story Webnovel Play and creation workbench release: pushes Inoks Story Webnovel from a "write the next chapter automatically" pipeline tool toward a more complete Story Creation AI Agent. Long-form novels, short fiction, fanfic, spinoffs, style imitation, continuation, covers, and open-world interaction now share the same Studio Chat / CLI / TUI interaction core, with systematic work on instruction following, context management, and the visual experience.

### Major Features

- Added the **Inoks Story Webnovel Play** open-world / branching interaction entry: free-form actions, clickable choices, world contracts, non-fixed time advancement, character agents, item / evidence / relationship state, HUD, and automatic illustration
...
- Model settings, cover services, aggregator API entries, and error messages are further separated: Inoks Story Webnovel execution errors, model provider errors, and image generation errors are no longer mixed together
...
- Long-form chapter writing gains the `writing.reviewRetries` project setting for automatic review/revision passes; the default remains 1, and you can run `inoks-story config set writing.reviewRetries 3` when stronger revision is needed
...
- Chat can directly edit generated text artifacts inside the project, which is handy for adjusting chapters, cover prompts, synopses, and other files before continuing with the Inoks Story Webnovel writing chain
...
- **Windows Studio startup fix**: `inoks-story studio` no longer crashes on Windows because the absolute-path loader was treated as an invalid ESM URL
- **Bootstrap projects fall back to env config**: Studio projects auto-initialized in an empty directory fall back to the global `.inoks-story-webnovel/.env` when no service is configured; `book create` no longer dies immediately on a missing key
...
- **Start directly from an empty directory**: `inoks-story` / `inoks-story studio` now auto-initialize a minimal project skeleton and start Studio, no longer requiring an explicit `init` first
...
Studio 2.0 official release. `inoks-story` now starts Studio directly by default; the local web workbench becomes the main entry, and the TUI is kept as `inoks-story tui`.

### New Features

- **Studio 2.0 as the default entry**: `inoks-story` starts Studio directly; the home page, provider management, and writing workbench are unified as the new main interaction entry
...
- **Config source switching**: Studio adds an explicit switch between `.env` and Studio configuration, no longer passively overridden by `INOKS_STORY_LLM_*` in the directory
...
- Fixed `inoks-story` starting Studio throwing the `llm.model` validation error directly when no default model was set
...
Unified interaction core — the TUI, Studio, `inoks-story interact`, and the OpenClaw Skill share the same natural-language understanding and execution runtime.
...
- **Ink TUI dashboard**: `inoks-story` opens a full-screen Ink + React dashboard directly, with conversational creation, slash-command Tab completion, themed animations (writing/auditing/revising/planning each with their own), and bilingual Chinese/English i18n
...
- **`inoks-story interact --json`**: the shared interaction JSON entry, returning request / response / session / events, callable directly by OpenClaw and external agents
...
- **Review Reject rollback**: `inoks-story review reject` rolls state back to the snapshot before the rejected chapter, discarding downstream chapters and memory indexes
- **State Validation Recovery**: failed state validation automatically retries the settler; if it still fails, the state is saved in degraded mode, with `inoks-story write repair-state` for manual repair
...
- **Custom HTTP headers**: the `INOKS_STORY_LLM_HEADERS` environment variable injects custom HTTP headers
...
Inoks Story Webnovel Studio + stability hardening. Upgraded from a CLI tool to CLI + web workbench.

### Inoks Story Webnovel Studio

- `inoks-story studio` starts the local web workbench (Vite + React + Hono, default port 4567)
...
- Added the `inoks-story eval` command — structured quality evaluation report
...
- The user's `INOKS_STORY_LLM_MAX_TOKENS` now takes effect as the global cap (#87)
...
- `inoks-story book delete <id>`: delete a book and all its data (`--force` skips confirmation)
- `inoks-story status --chapters`: shows per-chapter status and critical issues for failed chapters
...
- `inoks-story up` automatically writes `inoks-story-webnovel.log`; daemon restarts are traceable
...
- `inoks-story doctor` provides targeted hints on failure (check the baseUrl, try stream:false, check the API key)
...
Import an existing novel (single file or a chapter directory) into Inoks Story Webnovel; the system splits chapters automatically and reverse-engineers the full set of truth files (world state, hooks, character matrix, etc.), after which `write next` continues directly.

```bash
inoks-story import chapters 我的小说 --from 已有章节/        # import from a directory
inoks-story import chapters 我的小说 --from 全书.txt          # import from a single file (auto-split on "第X章")
inoks-story import chapters 我的小说 --from 全书.txt --split "Chapter\\s+\\d+"  # custom chapter-split regex
inoks-story write next 我的小说                               # continue seamlessly
```
...
```bash
inoks-story import canon 烈焰前传 --from 吞天魔帝   # import the parent canon into the spinoff
inoks-story write next 烈焰前传                     # the writer automatically reads the canon constraints
```
...
```bash
inoks-story style analyze 参考小说.txt                     # analyze: sentence length, TTR, rhetorical features
inoks-story style import 参考小说.txt 吞天魔帝 --name 某作者  # import the style into a book
```
...
```bash
inoks-story config set-model writer gpt-4o-mini                                    # simple model override
inoks-story config set-model auditor gemini-2.5-flash \
  --base-url https://generativelanguage.googleapis.com/v1beta/openai \
  --provider openai \
  --api-key-env GEMINI_API_KEY                                                # route through the Gemini API
inoks-story config set-model reviser claude-sonnet-4-20250514 \
  --base-url https://api.anthropic.com \
  --provider anthropic \
  --api-key-env ANTHROPIC_API_KEY                                             # route through the Anthropic API
inoks-story config show-models                                                      # view the full routing picture
```
...
```bash
inoks-story analytics 吞天魔帝          # audit pass rate, most frequent issue categories, chapters with the most issues
inoks-story analytics 吞天魔帝 --json   # structured output
```
...
- `inoks-story export` supports `--format md`, `--output <path>`, `--approved-only`
...
| AIGC detection API | External API integration (GPTZero / Originality / custom endpoints), the `inoks-story detect` command |
...
| Detection feedback loop | `detection_history.json` records every detection/rewrite result; `inoks-story detect --stats` shows statistics |

```bash
inoks-story style analyze reference.txt         # analyze the reference text's style
inoks-story style import reference.txt 吞天魔帝  # import the style into a book
inoks-story detect 吞天魔帝 --all               # whole-book AIGC detection
inoks-story detect --stats                      # detection statistics
```
...
```bash
inoks-story book create --title "吞天魔帝" --genre xuanhuan
```

Genre rules can be inspected, copied into a project for editing, or created from scratch:

```bash
inoks-story genre list                      # view all genres
inoks-story genre show xuanhuan             # view the complete xuanhuan rules
inoks-story genre copy xuanhuan             # copy into the project; edit freely
inoks-story genre create wuxia --name 武侠   # create a new genre from scratch
```
...
- `inoks-story update` for one-command updates, `inoks-story init` supports initializing the current directory
- API errors come with Chinese diagnostic hints, and `inoks-story doctor` includes an API connectivity test
